...
import cn.weitee.erp.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentPageReqVO;
import cn.weitee.erp.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentProcessRespVO;
import cn.weitee.erp.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentSaveReqVO;
import cn.weitee.erp.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentUpdateStatusReqVO;
...
import cn.weitee.erp.module.ai.dal.dataobject.knowledge.AiKnowledgeSegmentDO;
import cn.weitee.erp.module.ai.dal.mysql.knowledge.AiKnowledgeSegmentMapper;
import cn.weitee.erp.module.ai.enums.AiDocumentSplitStrategyEnum;
...
import cn.weitee.erp.module.ai.service.knowledge.splitter.MarkdownQaSplitter;
import cn.weitee.erp.module.ai.service.knowledge.splitter.SemanticTextSplitter;
...
import static cn.weitee.erp.module.ai.enums.ErrorCodeConstants.*;
...
public class AiKnowledgeSegmentServiceImpl implements AiKnowledgeSegmentService {
    ...
    @Resource
    private AiKnowledgeSegmentMapper aiKnowledgeSegmentMapper;
    ...
    public PageResult<AiKnowledgeSegmentDO> getKnowledgeSegmentPage(AiKnowledgeSegmentPageReqVO pageReqVO) {
        return aiKnowledgeSegmentMapper.selectPage(pageReqVO);
    }
    ...
    public void deleteKnowledgeSegmentByDocumentId(Long documentId) {
        // 1. 查询需要删除的段落
        List<AiKnowledgeSegmentDO> segments = aiKnowledgeSegmentMapper.selectListByDocumentId(documentId);
        if (CollUtil.isEmpty(segments)) {
            return;
        }

        // 2. 批量删除段落记录
        aiKnowledgeSegmentMapper.deleteByIds(convertList(segments, AiKnowledgeSegmentDO::getId));
    ...
    public void reindexKnowledgeSegmentByKnowledgeId(Long knowledgeId) {
        ...
        List<AiKnowledgeSegmentDO> segments = aiKnowledgeSegmentMapper.selectListByKnowledgeIdAndStatus(
    ...
    private void writeVectorStore(VectorStore vectorStore, AiKnowledgeSegmentDO segmentDO, Document segment) {
        ...
        // 2. 更新向量 ID
        aiKnowledgeSegmentMapper.updateById(new AiKnowledgeSegmentDO().setId(segmentDO.getId()).setVectorId(segment.getId()));
    }
    ...
    private void deleteVectorStore(VectorStore vectorStore, AiKnowledgeSegmentDO segmentDO) {
        ...
        aiKnowledgeSegmentMapper.updateById(new AiKnowledgeSegmentDO().setId(segmentDO.getId())
                .setVectorId(AiKnowledgeSegmentDO.VECTOR_ID_EMPTY));
    ...
    public List<AiKnowledgeSegmentSearchRespBO> searchKnowledgeSegment(AiKnowledgeSegmentSearchReqBO reqBO) {
        ...
        // 3.1 段落召回
        List<AiKnowledgeSegmentDO> segments = aiKnowledgeSegmentMapper
                .selectListByVectorIds(convertList(documents, Document::getId));
        if (CollUtil.isEmpty(segments)) {
            return ListUtil.empty();
        }
        // 3.2 增加召回次数
        aiKnowledgeSegmentMapper.updateRetrievalCountIncrByIds(convertList(segments, AiKnowledgeSegmentDO::getId));
    ...
    private AiKnowledgeSegmentDO validateKnowledgeSegmentExists(Long id) {
        AiKnowledgeSegmentDO knowledgeSegment = aiKnowledgeSegmentMapper.selectById(id);
    ...
    public List<AiKnowledgeSegmentProcessRespVO> getKnowledgeSegmentProcessList(List<Long> documentIds) {
        ...
        return aiKnowledgeSegmentMapper.selectProcessList(documentIds);
    }
    ...
    public AiKnowledgeSegmentDO getKnowledgeSegment(Long id) {
        return aiKnowledgeSegmentMapper.selectById(id);
    }
    ...
    public List<AiKnowledgeSegmentDO> getKnowledgeSegmentList(Collection<Long> ids) {
        ...
        return aiKnowledgeSegmentMapper.selectByIds(ids);
    }

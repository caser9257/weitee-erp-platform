...
import cn.weitee.erp.module.ai.controller.admin.chat.vo.message.AiChatMessageRespVO;
...
import cn.weitee.erp.module.ai.dal.dataobject.knowledge.AiKnowledgeDocumentDO;
import cn.weitee.erp.module.ai.dal.dataobject.model.AiChatRoleDO;
import cn.weitee.erp.module.ai.dal.dataobject.model.AiModelDO;
import cn.weitee.erp.module.ai.dal.dataobject.model.AiToolDO;
import cn.weitee.erp.module.ai.dal.mysql.chat.AiChatMessageMapper;
import cn.weitee.erp.module.ai.enums.ErrorCodeConstants;
import cn.weitee.erp.module.ai.enums.model.AiPlatformEnum;
import cn.weitee.erp.module.ai.framework.ai.core.webserch.AiWebSearchClient;
import cn.weitee.erp.module.ai.framework.ai.core.webserch.AiWebSearchRequest;
import cn.weitee.erp.module.ai.framework.ai.core.webserch.AiWebSearchResponse;
import cn.weitee.erp.module.ai.service.knowledge.AiKnowledgeDocumentService;
import cn.weitee.erp.module.ai.service.knowledge.AiKnowledgeSegmentService;
import cn.weitee.erp.module.ai.service.knowledge.bo.AiKnowledgeSegmentSearchReqBO;
import cn.weitee.erp.module.ai.service.knowledge.bo.AiKnowledgeSegmentSearchRespBO;
import cn.weitee.erp.module.ai.service.model.AiChatRoleService;
import cn.weitee.erp.module.ai.service.model.AiModelService;
import cn.weitee.erp.module.ai.service.model.AiToolService;
import cn.weitee.erp.module.ai.util.AiUtils;
import cn.weitee.erp.module.ai.util.FileTypeUtils;
...
import static cn.weitee.erp.framework.common.pojo.CommonResult.error;
...
import static cn.weitee.erp.module.ai.enums.ErrorCodeConstants.CHAT_CONVERSATION_NOT_EXISTS;
import static cn.weitee.erp.module.ai.enums.ErrorCodeConstants.CHAT_MESSAGE_NOT_EXIST;
...
public class AiChatMessageServiceImpl implements AiChatMessageService {
    ...
    @Autowired(required = false) // 由于 weitee.ai.web-search.enable 配置项，可以关闭 AiWebSearchClient 的功能，所以这里只能不强制注入
    private AiWebSearchClient webSearchClient;

    @SuppressWarnings("SpringJavaAutowiredFieldsWarningInspection")
    @Autowired(required = false) // 由于 weitee.ai.mcp.client.enable 配置项，可以关闭 McpSyncClient 的功能，所以这里只能不强制注入
    ...
    public AiChatMessageSendRespVO sendMessage(AiChatMessageSendReqVO sendReqVO, Long userId) {
        ...
        List<AiChatMessageDO> historyMessages = aiChatMessageMapper.selectListByConversationId(conversation.getId());
        // 1.2 校验模型
        ...
        return streamResponse.map(chunk -> {
            ...
            log.error("[sendChatMessageStream][userId({}) sendReqVO({}) 发生异常]", userId, sendReqVO, throwable);
            {
                ...
                if (StrUtil.isNotEmpty(contentBuffer)) {
                    ...
                    // 否则，则进行删除
                    aiChatMessageMapper.deleteById(assistantMessage.getId());
                }
            }
        }).doOnCancel(() -> {
    ...
    public void deleteChatMessage(Long id, Long userId) {
        // 1. 校验消息存在
        AiChatMessageDO message = aiChatMessageMapper.selectById(id);
    ...
    public void deleteChatMessageByConversationId(Long conversationId, Long userId) {
        // 1. 校验消息存在
        List<AiChatMessageDO> messages = aiChatMessageMapper.selectListByConversationId(conversationId);
        if (CollUtil.isEmpty(messages) || ObjUtil.notEqual(messages.get(0).getUserId(), userId)) {
            throw exception(CHAT_MESSAGE_NOT_EXIST);
        }
        // 2. 执行删除
        aiChatMessageMapper.deleteByIds(convertList(messages, AiChatMessageDO::getId));
    }
    ...
    public Map<Long, Integer> getChatMessageCountMap(Collection<Long> conversationIds) {
        return aiChatMessageMapper.selectCountMapByConversationId(conversationIds);
    }

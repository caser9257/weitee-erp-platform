...
public class PayRefundServiceImpl implements PayRefundService {
    ...
    public Long getRefundCountByAppId(Long appId) {
        return payRefundMapper.selectCountByAppId(appId);
    }
    ...
    public Long createRefund(PayRefundCreateReqDTO reqDTO) {
        ...
        // 1.4 校验退款订单是否已经存在
        PayRefundDO refund = payRefundMapper.selectByAppIdAndMerchantRefundId(
    ...
    private PayOrderDO validatePayOrderCanRefund(PayRefundCreateReqDTO reqDTO, Long appId) {
        ...
        // 是否有退款中的订单
        if (payRefundMapper.selectCountByAppIdAndOrderId(appId, order.getId(),
    ...
    private void notifyRefundSuccess(PayChannelDO channel, PayRefundRespDTO notify) {
        // 1.1 查询 PayRefundDO
        PayRefundDO refund = payRefundMapper.selectByAppIdAndNo(
        ...
        int updateCounts = payRefundMapper.updateByIdAndStatus(refund.getId(), refund.getStatus(), updateRefundObj);
    ...
    public int syncRefund() {
        // 1. 查询指定创建时间内的待退款订单
        List<PayRefundDO> refunds = payRefundMapper.selectListByStatus(PayRefundStatusEnum.WAITING.getStatus());

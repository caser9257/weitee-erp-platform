...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    void validateOrderActuallyPaid(Long id) {
        List<PayOrderExtensionDO> orderExtensions = payOrderExtensionMapper.selectListByOrderId(id);
    ...
    public PayOrderExtensionDO getOrderExtensionByNo(String no) {
        return payOrderExtensionMapper.selectByNo(no);
    }

    @Override
    public int syncOrder(LocalDateTime minCreateTime) {
        // 1. 查询指定创建时间前的待支付订单
        List<PayOrderExtensionDO> orderExtensions = payOrderExtensionMapper.selectListByStatusAndCreateTimeGe(
    ...
    public void syncOrderQuietly(Long id) {
        // 1. 查询待支付订单
        List<PayOrderExtensionDO> orderExtensions = payOrderExtensionMapper.selectListByOrderIdAndStatus(id,
    ...
    private boolean expireOrder(PayOrderDO order) {
        try {
            ...
            List<PayOrderExtensionDO> orderExtensions = payOrderExtensionMapper.selectListByOrderId(order.getId());
            for (PayOrderExtensionDO orderExtension : orderExtensions) {
                ...
                if (payOrderExtensionMapper.updateByIdAndStatus(orderExtension.getId(), PayOrderStatusEnum.WAITING.getStatus(),
                        updateObj) == 0) {

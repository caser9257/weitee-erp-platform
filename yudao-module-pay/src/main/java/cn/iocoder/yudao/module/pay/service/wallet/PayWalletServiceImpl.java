...
public class PayWalletServiceImpl implements PayWalletService {
    ...
    public PayWalletDO getOrCreateWallet(Long userId, Integer userType) {
        ...
        if (wallet == null) {
            ...
            wallet = lockRedisDAO.lock(userId, UPDATE_TIMEOUT_MILLIS, () -> {
                PayWalletDO newWallet = payWalletMapper.selectByUserIdAndType(userId, userType);
                if (newWallet == null) {
    ...
    public PayWalletTransactionDO reduceWalletBalance(Long walletId, Long bizId,
        ...
        return lockRedisDAO.lock(walletId, UPDATE_TIMEOUT_MILLIS, () -> {
            ...
            switch (bizType) {
                case PAYMENT: {
                    updateCounts = payWalletMapper.updateWhenConsumption(payWallet.getId(), price);
                    break;
                }
                case RECHARGE_REFUND: {
                    updateCounts = payWalletMapper.updateWhenRechargeRefund(payWallet.getId(), price);
                    break;
                }
                default: {
                    // TODO 其它类型待实现
    ...
    public PayWalletTransactionDO addWalletBalance(Long walletId, String bizId,
        ...
        return lockRedisDAO.lock(walletId, UPDATE_TIMEOUT_MILLIS, () -> {
            ...
            switch (bizType) {
                ...
                case TRANSFER: // 分佣提现
                    payWalletMapper.updateWhenAdd(payWallet.getId(), price);
                    break;
                default: {

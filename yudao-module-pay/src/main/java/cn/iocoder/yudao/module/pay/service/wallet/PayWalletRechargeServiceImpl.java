...
public class PayWalletRechargeServiceImpl implements PayWalletRechargeService {
    ...
    public PayWalletRechargeDO createWalletRecharge(Long userId, Integer userType, String userIp,
        ...
        // 2.2 更新钱包充值记录中支付订单
        payWalletRechargeMapper.updateById(new PayWalletRechargeDO().setId(recharge.getId()).setPayOrderId(payOrderId));
    ...
    public PageResult<PayWalletRechargeDO> getWalletRechargePackagePage(Long userId, Integer userType,
        ...
        PayWalletDO wallet = payWalletService.getOrCreateWallet(userId, userType);
        return payWalletRechargeMapper.selectPage(pageReqVO, wallet.getId(), payStatus);
    }
    ...
    public void updateWalletRechargerPaid(Long id, Long payOrderId) {
        // 1.1 校验钱包充值是否存在
        PayWalletRechargeDO recharge = payWalletRechargeMapper.selectById(id);
        if (recharge == null) {
        ...
        // 3. 更新钱包充值的支付状态
        int updateCount = payWalletRechargeMapper.updateByIdAndPaid(id, false,
    ...
    public void refundWalletRecharge(Long id, String userIp) {
        // 1.1 获取钱包充值记录
        PayWalletRechargeDO walletRecharge = payWalletRechargeMapper.selectById(id);
        ...
        // 4. 更新充值记录退款单号
        payWalletRechargeMapper.updateById(new PayWalletRechargeDO().setPayRefundId(payRefundId)
    ...
    public void updateWalletRechargeRefunded(Long id, String refundId, Long payRefundId) {
        ...
        // 3. 更新钱包充值的退款字段
        payWalletRechargeMapper.updateByIdAndRefunded(id, WAITING.getStatus(), updateObj);
    }

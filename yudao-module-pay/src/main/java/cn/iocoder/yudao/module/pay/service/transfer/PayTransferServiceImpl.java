...
public class PayTransferServiceImpl implements PayTransferService {
    ...
    public PayTransferCreateRespDTO createTransfer(PayTransferCreateReqDTO reqDTO) {
        ...
        if (transfer == null) {
            ...
            payTransferMapper.updateByIdAndStatus(transfer.getId(), transfer.getStatus(),
    ...
    private PayTransferDO validateTransferCanCreate(PayTransferCreateReqDTO reqDTO, Long appId) {
        PayTransferDO transfer = payTransferMapper.selectByAppIdAndMerchantOrderId(appId, reqDTO.getMerchantTransferId());
        if (transfer != null) {
            // 只有转账单状态为关闭，才能再次发起转账
    ...
    private void notifyTransferProgressing(PayChannelDO channel, PayTransferRespDTO notify) {
        // 1. 校验
        PayTransferDO transfer = payTransferMapper.selectByAppIdAndNo(channel.getAppId(), notify.getOutTransferNo());
        if (transfer == null) {
        ...
        // 2. 更新状态
        int updateCounts = payTransferMapper.updateByIdAndStatus(transfer.getId(),
    ...
    private void notifyTransferClosed(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        // 2. 更新状态
        int updateCount = payTransferMapper.updateByIdAndStatus(transfer.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus(), PayTransferStatusEnum.PROCESSING.getStatus()),
    ...
    public int syncTransfer() {
        List<PayTransferDO> list = payTransferMapper.selectListByStatus(CollUtil.newArrayList(

package cn.weitee.erp.module.iot.core.messagebus.config;

import cn.weitee.erp.framework.mq.redis.core.RedisMQTemplate;
import cn.weitee.erp.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import cn.weitee.erp.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import cn.weitee.erp.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import cn.weitee.erp.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import cn.weitee.erp.module.iot.core.messagebus.core.IotMessageBus;
import cn.weitee.erp.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import cn.weitee.erp.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import cn.weitee.erp.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
import cn.weitee.erp.module.iot.core.mq.producer.IotDeviceMessageProducer;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "weitee.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "weitee.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    /**
     * 特殊：由于 WeiteeRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see cn.weitee.erp.framework.mq.redis.config.WeiteeRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "weitee.iot.message-bus", name = "type", havingValue = "redis")

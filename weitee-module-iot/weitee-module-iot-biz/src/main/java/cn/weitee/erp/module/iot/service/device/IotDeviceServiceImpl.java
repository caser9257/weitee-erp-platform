...
import cn.weitee.erp.module.iot.core.biz.dto.IotDeviceAuthReqDTO;
import cn.weitee.erp.module.iot.core.biz.dto.IotSubDeviceRegisterFullReqDTO;
import cn.weitee.erp.module.iot.core.enums.IotDeviceMessageMethodEnum;
import cn.weitee.erp.module.iot.core.enums.device.IotDeviceStateEnum;
...
import cn.weitee.erp.module.iot.core.topic.auth.IotDeviceRegisterReqDTO;
import cn.weitee.erp.module.iot.core.topic.auth.IotDeviceRegisterRespDTO;
import cn.weitee.erp.module.iot.core.topic.auth.IotSubDeviceRegisterReqDTO;
import cn.weitee.erp.module.iot.core.topic.auth.IotSubDeviceRegisterRespDTO;
import cn.weitee.erp.module.iot.core.topic.topo.IotDeviceTopoAddReqDTO;
import cn.weitee.erp.module.iot.core.topic.topo.IotDeviceTopoChangeReqDTO;
import cn.weitee.erp.module.iot.core.topic.topo.IotDeviceTopoDeleteReqDTO;
import cn.weitee.erp.module.iot.core.topic.topo.IotDeviceTopoGetRespDTO;
import cn.weitee.erp.module.iot.core.util.IotDeviceAuthUtils;
import cn.weitee.erp.module.iot.core.util.IotProductAuthUtils;
...
import cn.weitee.erp.module.iot.dal.mysql.device.IotDeviceMapper;
import cn.weitee.erp.module.iot.dal.redis.RedisKeyConstants;
import cn.weitee.erp.module.iot.enums.product.IotProductDeviceTypeEnum;
import cn.weitee.erp.module.iot.service.device.message.IotDeviceMessageService;
import cn.weitee.erp.module.iot.service.product.IotProductService;
...
public class IotDeviceServiceImpl implements IotDeviceService {
    ...
    private void validateCreateDeviceParam(String productKey, String deviceName,
        ...
        // 校验设备名称在同一产品下是否唯一
        if (iotDeviceMapper.selectByProductKeyAndDeviceName(productKey, deviceName) != null) {
    ...
    private void validateSerialNumberUnique(String serialNumber, Long excludeId) {
        ...
        IotDeviceDO existDevice = iotDeviceMapper.selectBySerialNumber(serialNumber);
    ...
    public void updateDeviceGroup(IotDeviceUpdateGroupReqVO updateReqVO) {
        // 1.1 校验设备存在
        List<IotDeviceDO> devices = iotDeviceMapper.selectByIds(updateReqVO.getIds());
        ...
        // 3. 更新设备分组
        iotDeviceMapper.updateBatch(convertList(devices, device -> new IotDeviceDO()
    ...
    public void deleteDevice(Long id) {
        ...
        if (IotProductDeviceTypeEnum.isGateway(device.getDeviceType())
                && iotDeviceMapper.selectCountByGatewayId(id) > 0) {
    ...
    public void deleteDeviceList(Collection<Long> ids) {
        ...
        List<IotDeviceDO> devices = iotDeviceMapper.selectByIds(ids);
        if (CollUtil.isEmpty(devices)) {
            return;
        }
        // 1.2 如果是网关设备，检查是否有子设备绑定
        for (IotDeviceDO device : devices) {
            if (IotProductDeviceTypeEnum.isGateway(device.getDeviceType())
                    && iotDeviceMapper.selectCountByGatewayId(device.getId()) > 0) {
                throw exception(DEVICE_GATEWAY_HAS_SUB);
            }
        }
    ...
    public IotDeviceDO getDeviceFromCache(String productKey, String deviceName) {
        return iotDeviceMapper.selectByProductKeyAndDeviceName(productKey, deviceName);
    }
    ...
    public List<IotDeviceDO> getDeviceListByCondition(@Nullable Integer deviceType, @Nullable Long productId) {
        return iotDeviceMapper.selectListByCondition(deviceType, productId);
    }

    @Override
    public List<IotDeviceDO> getDeviceListByState(Integer state) {
        return iotDeviceMapper.selectListByState(state);
    }

    @Override
    public List<IotDeviceDO> getDeviceListByProductId(Long productId) {
        return iotDeviceMapper.selectListByProductId(productId);
    }
    ...
    private void handleGatewayOffline(IotDeviceDO gatewayDevice) {
        List<IotDeviceDO> subDevices = iotDeviceMapper.selectListByGatewayId(gatewayDevice.getId());
        ...
        importDevices.forEach(importDevice -> {
            try {
                ...
                if (StrUtil.isNotEmpty(importDevice.getParentDeviceName())) {
                    IotDeviceDO gatewayDevice = iotDeviceMapper.selectByDeviceName(importDevice.getParentDeviceName());
                ...
                // 2.2.1 判断如果不存在，在进行插入
                IotDeviceDO existDevice = iotDeviceMapper.selectByDeviceName(importDevice.getDeviceName());
                if (existDevice == null) {
    ...
    public List<IotDeviceDO> getDeviceListByProductKeyAndNames(String productKey, List<String> deviceNames) {
        ...
        return iotDeviceMapper.selectByProductKeyAndDeviceNames(productKey, deviceNames);
    }
    ...
    public List<IotDeviceDO> getDeviceList(Collection<Long> ids) {
        ...
        return iotDeviceMapper.selectByIds(ids);
    }
    ...
    public void updateDeviceLocation(IotDeviceDO device, BigDecimal longitude, BigDecimal latitude) {
        // 1. 更新定位信息
        iotDeviceMapper.updateById(new IotDeviceDO().setId(device.getId())
    ...
    public List<IotDeviceDO> getDeviceListByHasLocation() {
        return iotDeviceMapper.selectListByHasLocation();
    }
    ...
    public void bindDeviceGateway(Collection<Long> subIds, Long gatewayId) {
        ...
        validateGatewayDeviceExists(gatewayId);
        // 1.2 校验每个设备是否可绑定
        List<IotDeviceDO> devices = iotDeviceMapper.selectByIds(subIds);
        for (IotDeviceDO device : devices) {
        ...
        iotDeviceMapper.updateBatch(updateList);
    ...
    public void unbindDeviceGateway(Collection<Long> subIds, Long gatewayId) {
        ...
        iotDeviceMapper.updateGatewayIdBatch(convertList(devices, IotDeviceDO::getId), null);
    ...
    public PageResult<IotDeviceDO> getUnboundSubDevicePage(IotDevicePageReqVO pageReqVO) {
        return iotDeviceMapper.selectUnboundSubDevicePage(pageReqVO);
    }

    @Override
    public List<IotDeviceDO> getDeviceListByGatewayId(Long gatewayId) {
        return iotDeviceMapper.selectListByGatewayId(gatewayId);
    }
    ...
    private IotDeviceDO addDeviceTopo(IotDeviceDO gatewayDevice, IotDeviceAuthReqDTO subDeviceAuth) {
        ...
        // 2. 更新数据库
        iotDeviceMapper.updateById(new IotDeviceDO().setId(subDevice.getId()).setGatewayId(gatewayDevice.getId()));
    ...
    private void deleteDeviceTopo(IotDeviceDO gatewayDevice, IotDeviceIdentity subDeviceIdentity) {
        ...
        iotDeviceMapper.updateGatewayIdBatch(singletonList(subDevice.getId()), null);
    ...
    private void sendTopoChangeNotify(Long gatewayId, Integer status, List<IotDeviceDO> subDevices) {
        ...
        // 1. 获取网关设备
        IotDeviceDO gatewayDevice = iotDeviceMapper.selectById(gatewayId);
        if (gatewayDevice == null) {
    ...
    public IotDeviceRegisterRespDTO registerDevice(IotDeviceRegisterReqDTO reqDTO) {
        ...
        log.info("[registerDevice][产品({}) 自动创建设备({})]", reqDTO.getProductKey(), reqDTO.getDeviceName());
        // 2.2 返回设备密钥
    ...
    private IotDeviceDO registerSubDevice0(IotDeviceDO gatewayDevice, IotSubDeviceRegisterReqDTO params) {
        ...
        if (existDevice.getGatewayId() == null) {
            // 2.1 更新数据库
            iotDeviceMapper.updateById(new IotDeviceDO().setId(existDevice.getId()).setGatewayId(gatewayDevice.getId()));
            // 2.2 清空对应缓存

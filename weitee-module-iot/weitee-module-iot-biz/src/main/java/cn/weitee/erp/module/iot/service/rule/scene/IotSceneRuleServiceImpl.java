package cn.weitee.erp.module.iot.service.rule.scene;
...
import cn.weitee.erp.module.iot.controller.admin.rule.vo.scene.IotSceneRulePageReqVO;
import cn.weitee.erp.module.iot.controller.admin.rule.vo.scene.IotSceneRuleSaveReqVO;
...
import cn.weitee.erp.module.iot.dal.mysql.rule.IotSceneRuleMapper;
import cn.weitee.erp.module.iot.dal.redis.RedisKeyConstants;
import cn.weitee.erp.module.iot.enums.rule.IotSceneRuleTriggerTypeEnum;
...
import cn.weitee.erp.module.iot.service.rule.scene.action.IotSceneRuleAction;
import cn.weitee.erp.module.iot.service.rule.scene.matcher.IotSceneRuleMatcherManager;
import cn.weitee.erp.module.iot.service.rule.scene.timer.IotSceneRuleTimerHandler;
import cn.weitee.erp.module.iot.service.rule.scene.timer.IotTimerConditionEvaluator;
...
import static cn.weitee.erp.module.iot.enums.ErrorCodeConstants.RULE_SCENE_NOT_EXISTS;
...
public class IotSceneRuleServiceImpl implements IotSceneRuleService {
    ...
    @Resource
    private IotSceneRuleMapper iotSceneRuleMapper;
    ...
    public Long createSceneRule(IotSceneRuleSaveReqVO createReqVO) {
        ...
        iotSceneRuleMapper.insert(sceneRule);
    ...
    public void updateSceneRule(IotSceneRuleSaveReqVO updateReqVO) {
        ...
        iotSceneRuleMapper.updateById(updateObj);
    ...
    public void updateSceneRuleStatus(Long id, Integer status) {
        ...
        if (CommonStatusEnum.isEnable(status)) {
            ...
            IotSceneRuleDO sceneRule = iotSceneRuleMapper.selectById(id);
            if (sceneRule != null) {
                timerHandler.registerTimerTriggers(sceneRule);
            }
        } else {
    ...
    public void deleteSceneRule(Long id) {
        ...
        // 2. 删除
        iotSceneRuleMapper.deleteById(id);
    ...
    private void validateSceneRuleExists(Long id) {
        if (iotSceneRuleMapper.selectById(id) == null) {
    ...
    public IotSceneRuleDO getSceneRule(Long id) {
        return iotSceneRuleMapper.selectById(id);
    }

    @Override
    public PageResult<IotSceneRuleDO> getSceneRulePage(IotSceneRulePageReqVO pageReqVO) {
        return iotSceneRuleMapper.selectPage(pageReqVO);
    }
    ...
    public void validateSceneRuleList(Collection<Long> ids) {
        ...
        // 批量查询存在的规则场景
        List<IotSceneRuleDO> existingScenes = iotSceneRuleMapper.selectByIds(ids);
    ...
    public List<IotSceneRuleDO> getSceneRuleListByStatus(Integer status) {
        return iotSceneRuleMapper.selectListByStatus(status);
    }
    ...
    public List<IotSceneRuleDO> getSceneRuleListByProductIdAndDeviceIdFromCache(Long productId, Long deviceId) {
        // 1. 查询启用状态的规则场景
        List<IotSceneRuleDO> enabledList = iotSceneRuleMapper.selectList(IotSceneRuleDO::getStatus, CommonStatusEnum.ENABLE.getStatus());
    ...
    public void executeSceneRuleByTimer(Long id) {
        // 1.1 获得规则场景
        IotSceneRuleDO scene = iotSceneRuleMapper.selectById(id);
        if (scene == null) {
    ...
    private void updateLastTriggerTime(Long id) {
        try {
            iotSceneRuleMapper.updateById(new IotSceneRuleDO().setId(id).setLastTriggerTime(LocalDateTime.now()));
        } catch (Exception e) {

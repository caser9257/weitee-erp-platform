package cn.weitee.erp.module.iot.service.ota;
...
import cn.weitee.erp.module.iot.controller.admin.ota.vo.task.record.IotOtaTaskRecordPageReqVO;
...
import cn.weitee.erp.module.iot.core.topic.ota.IotDeviceOtaProgressReqDTO;
import cn.weitee.erp.module.iot.core.topic.ota.IotDeviceOtaUpgradeReqDTO;
import cn.weitee.erp.module.iot.dal.dataobject.device.IotDeviceDO;
import cn.weitee.erp.module.iot.dal.dataobject.ota.IotOtaFirmwareDO;
import cn.weitee.erp.module.iot.dal.dataobject.ota.IotOtaTaskRecordDO;
import cn.weitee.erp.module.iot.dal.mysql.ota.IotOtaTaskRecordMapper;
import cn.weitee.erp.module.iot.enums.ota.IotOtaTaskRecordStatusEnum;
...
public class IotOtaTaskRecordServiceImpl implements IotOtaTaskRecordService {
    ...
    @Resource
    private IotOtaTaskRecordMapper iotOtaTaskRecordMapper;
    ...
    public void createOtaTaskRecordList(List<IotDeviceDO> devices, Long firmwareId, Long taskId) {
        ...
        iotOtaTaskRecordMapper.insertBatch(records);
    }
    ...
    public Map<Integer, Long> getOtaTaskRecordStatusStatistics(Long firmwareId, Long taskId) {
        ...
        List<IotOtaTaskRecordDO> records = iotOtaTaskRecordMapper.selectListByFirmwareIdAndTaskId(firmwareId, taskId);
    ...
    public IotOtaTaskRecordDO getOtaTaskRecord(Long id) {
        return iotOtaTaskRecordMapper.selectById(id);
    }

    @Override
    public PageResult<IotOtaTaskRecordDO> getOtaTaskRecordPage(IotOtaTaskRecordPageReqVO pageReqVO) {
        return iotOtaTaskRecordMapper.selectPage(pageReqVO);
    }

    @Override
    public void cancelTaskRecordListByTaskId(Long taskId) {
        List<IotOtaTaskRecordDO> records = iotOtaTaskRecordMapper.selectListByTaskIdAndStatus(
        ...
        iotOtaTaskRecordMapper.updateListByIdAndStatus(ids, IotOtaTaskRecordStatusEnum.IN_PROCESS_STATUSES,
    ...
    public List<IotOtaTaskRecordDO> getOtaTaskRecordListByDeviceIdAndStatus(Set<Long> deviceIds, Set<Integer> statuses) {
        return iotOtaTaskRecordMapper.selectListByDeviceIdAndStatus(deviceIds, statuses);
    }

    @Override
    public List<IotOtaTaskRecordDO> getOtaRecordListByStatus(Integer status) {
        return iotOtaTaskRecordMapper.selectListByStatus(status);
    }
    ...
    public void cancelOtaTaskRecord(Long id) {
        ...
        // 2. 更新记录状态为取消
        int updateCount = iotOtaTaskRecordMapper.updateByIdAndStatus(record.getId(), IotOtaTaskRecordStatusEnum.IN_PROCESS_STATUSES,
    ...
    public boolean pushOtaTaskRecord(IotOtaTaskRecordDO record, IotOtaFirmwareDO fireware, IotDeviceDO device) {
        try {
            ...
            // 2. 更新 OTA 升级记录状态为进行中
            int updateCount = iotOtaTaskRecordMapper.updateByIdAndStatus(
            ...
            iotOtaTaskRecordMapper.updateById(IotOtaTaskRecordDO.builder().id(record.getId())
    ...
    private IotOtaTaskRecordDO validateUpgradeRecordExists(Long id) {
        IotOtaTaskRecordDO upgradeRecord = iotOtaTaskRecordMapper.selectById(id);
    ...
    public void updateOtaRecordProgress(IotDeviceDO device, IotDeviceMessage message) {
        ...
        // 1.2 查询 OTA 升级记录
        List<IotOtaTaskRecordDO> records = iotOtaTaskRecordMapper.selectListByDeviceIdAndStatus(
    ...
    private void checkAndUpdateOtaTaskStatus(Long taskId) {
        // 如果还有进行中的记录，直接返回
        Long inProcessCount = iotOtaTaskRecordMapper.selectCountByTaskIdAndStatus(
                taskId, IotOtaTaskRecordStatusEnum.IN_PROCESS_STATUSES);
        if (inProcessCount > 0) {
            return;
        }

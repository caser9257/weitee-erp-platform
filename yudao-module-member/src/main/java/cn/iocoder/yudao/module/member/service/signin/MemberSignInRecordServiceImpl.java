...
public class MemberSignInRecordServiceImpl implements MemberSignInRecordService {
    ...
    public AppMemberSignInRecordSummaryRespVO getSignInRecordSummary(Long userId) {
        ...
        // 2. 获取用户签到的记录数
        Long signCount = memberSignInRecordMapper.selectCountByUserId(userId);
        ...
        // 3. 校验当天是否有签到
        MemberSignInRecordDO lastRecord = memberSignInRecordMapper.selectLastRecordByUserId(userId);
        if (lastRecord == null) {
            return summary;
        }
    ...
    public MemberSignInRecordDO createSignRecord(Long userId) {
        ...
        // 3. 插入签到记录
        memberSignInRecordMapper.insert(record);

...
public class DeptServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testImportDeptList_create() {
        // mock 根部门下的负责人
        AdminUserDO leader = randomAdminUserDO(o -> {
            o.setUsername("dept-import-leader");
            o.setStatus(CommonStatusEnum.ENABLE.getStatus());
        });
        adminUserMapper.insert(leader);

        // 准备导入数据
        DeptImportExcelVO importVO = DeptImportExcelVO.builder()
                .name("导入测试部门")
                .parentName("")
                .sort(10)
        ...
        // 调用
        DeptImportRespVO respVO = deptService.importDeptList(Collections.singletonList(importVO), false);

        // 断言
        assertEquals(Collections.singletonList("导入测试部门"), respVO.getCreateDeptNames());
        assertTrue(respVO.getUpdateDeptNames().isEmpty());
        assertTrue(respVO.getFailureDeptNames().isEmpty());

        DeptDO dept = deptMapper.selectByParentIdAndName(DeptDO.PARENT_ID_ROOT, "导入测试部门");
        assertNotNull(dept);
        assertEquals(leader.getId(), dept.getLeaderUserId());
        assertEquals("13800138000", dept.getPhone());
        assertEquals("dept@test.com", dept.getEmail());
        assertEquals(CommonStatusEnum.ENABLE.getStatus(), dept.getStatus());
    }

    @Test
    public void testImportDeptList_update() {
        // mock 父部门和负责人
        DeptDO parentDept = randomPojo(DeptDO.class, o -> o.setName("导入父部门").setParentId(DeptDO.PARENT_ID_ROOT));
        deptMapper.insert(parentDept);
        AdminUserDO leader = randomAdminUserDO(o -> {
            o.setUsername("dept-update-leader");
            o.setStatus(CommonStatusEnum.ENABLE.getStatus());
        });
        adminUserMapper.insert(leader);
        DeptDO existDept = randomPojo(DeptDO.class, o -> {
            o.setName("导入更新部门");
            o.setParentId(parentDept.getId());
            o.setSort(1);
            o.setLeaderUserId(null);
            o.setPhone("");
            o.setEmail("");
            o.setStatus(CommonStatusEnum.DISABLE.getStatus());
        });
        deptMapper.insert(existDept);
        ...
        // 调用
        DeptImportRespVO respVO = deptService.importDeptList(Collections.singletonList(importVO), true);

        // 断言
        assertTrue(respVO.getCreateDeptNames().isEmpty());
        assertEquals(Collections.singletonList("导入更新部门"), respVO.getUpdateDeptNames());
        assertTrue(respVO.getFailureDeptNames().isEmpty());

        DeptDO dept = deptMapper.selectById(existDept.getId());
        assertEquals(99, dept.getSort());
        assertEquals(leader.getId(), dept.getLeaderUserId());
        assertEquals("13900139000", dept.getPhone());
        assertEquals("update@test.com", dept.getEmail());
        assertEquals(CommonStatusEnum.ENABLE.getStatus(), dept.getStatus());
    }
    ...
    private static AdminUserDO randomAdminUserDO(Consumer<AdminUserDO>... consumers) {
        ...
        Consumer<AdminUserDO>[] mergedConsumers = Arrays.copyOf(consumers, consumers.length + 1);
        System.arraycopy(mergedConsumers, 0, mergedConsumers, 1, consumers.length);
        mergedConsumers[0] = consumer;
        return randomPojo(AdminUserDO.class, mergedConsumers);
    }

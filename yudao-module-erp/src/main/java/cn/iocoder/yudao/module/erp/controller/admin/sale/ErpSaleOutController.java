...
public class ErpSaleOutController {
    ...
    private PageResult<ErpSaleOutRespVO> buildSaleOutVOPageResult(PageResult<ErpSaleOutDO> pageResult) {
        ...
        // 1.4 管理员信息
        Set<Long> creatorIds = convertSet(pageResult.getList(), saleOut -> parseUserId(saleOut.getCreator()));
        Map<Long, AdminUserRespDTO> userMap = CollUtil.isEmpty(creatorIds) ? Collections.emptyMap() : adminUserApi.getUserMap(creatorIds);
        // 2. 开始拼接
        return BeanUtils.toBean(pageResult, ErpSaleOutRespVO.class, saleOut -> {
            ...
            MapUtils.findAndThen(userMap, parseUserId(saleOut.getCreator()), user -> saleOut.setCreatorName(user.getNickname()));
        });
    }

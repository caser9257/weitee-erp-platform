...
public interface ErpFinancePaymentItemMapper extends BaseMapperX<ErpFinancePaymentItemDO> {
    ...
    default BigDecimal selectApprovedPaymentPriceSumByBizIdAndBizType(Long bizId, Integer bizType) {
        // SQL sum 查询
        List<Map<String, Object>> result = selectMaps(new QueryWrapper<ErpFinancePaymentItemDO>()
                ...
                .eq("biz_id", bizId)
                .eq("biz_type", bizType)
                .inSql("payment_id", "SELECT id FROM erp_finance_payment WHERE status = " + ErpAuditStatus.APPROVE.getStatus()));
        // 获得数量

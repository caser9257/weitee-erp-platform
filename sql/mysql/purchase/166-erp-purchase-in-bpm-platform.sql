-- 采购入库 BPM 场景、方案、版本和默认规则
-- 幂等：可重复执行；流程定义由应用启动时自动部署。
...
SELECT 'erp.purchase.in.submit', '采购入库审批', 'erp_purchase', 'purchase_in', 'submit', NULL,
       0, 1, '供应链模块-采购入库 BPM 审批', 'admin', NOW(), 'admin', NOW(), b'0'
WHERE NOT EXISTS (
    SELECT 1 FROM bpm_approval_scene
    WHERE scene_code = 'erp.purchase.in.submit' AND deleted = b'0'
);

SET @purchase_in_scene_id = (
...
SELECT 'erp.purchase.in.scheme.v1', '采购入库审批方案', 'erp_purchase', 'purchase_in',
       @purchase_in_scene_id, '采购入库默认审批方案', NULL, NULL, 0,
       'admin', NOW(), 'admin', NOW(), b'0'
WHERE @purchase_in_scene_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1 FROM bpm_approval_scheme
      WHERE code = 'erp.purchase.in.scheme.v1' AND deleted = b'0'
  );

SET @purchase_in_scheme_id = (
...
SELECT @purchase_in_scheme_id, 1, 30, NULL, 'CREATE', '{}', NULL,
       '初始版本', 'admin', NOW(), 'admin', NOW(), 'admin', NOW(), b'0'
WHERE @purchase_in_scheme_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1 FROM bpm_approval_scheme_version
      WHERE scheme_id = @purchase_in_scheme_id AND deleted = b'0'
  );

SET @purchase_in_version_id = (
...
SELECT @purchase_in_version_id, '默认规则', 'DEFAULT', 1, 1, NULL,
       'erp_purchase_in_approval', 1, 'admin', NOW(), 'admin', NOW(), b'0'
WHERE @purchase_in_version_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1 FROM bpm_approval_rule
      WHERE scheme_version_id = @purchase_in_version_id
...
SET rule_type = 'DEFAULT', priority = 1, is_default = 1, condition_json = NULL,
    process_json = 'erp_purchase_in_approval', enabled = 1, deleted = b'0',
    updater = 'admin', update_time = NOW()
WHERE scheme_version_id = @purchase_in_version_id
  AND rule_name = '默认规则';

UPDATE bpm_approval_scheme
SET active_version_id = @purchase_in_version_id,
    latest_version_id = @purchase_in_version_id,
    updater = 'admin', update_time = NOW()
WHERE id = @purchase_in_scheme_id;

UPDATE bpm_approval_scene
SET active_scheme_id = @purchase_in_scheme_id,
    updater = 'admin', update_time = NOW()
WHERE id = @purchase_in_scene_id;

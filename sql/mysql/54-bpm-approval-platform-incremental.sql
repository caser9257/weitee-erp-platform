...
CREATE TABLE IF NOT EXISTS `bpm_approval_scene` (
    ...
    `owner_user_id` BIGINT NOT NULL COMMENT '归属用户ID（配置管理员）',
...
-- 2. 为审批方案表添加字段（使用存储过程避免重复执行错误）
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS add_columns_if_not_exists()
BEGIN
    -- 检查并添加 owner_user_id 字段
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'bpm_approval_scheme' AND COLUMN_NAME = 'owner_user_id') THEN
    ...
    -- 检查并添加 scene_id 字段
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'bpm_approval_scheme' AND COLUMN_NAME = 'scene_id') THEN
        ALTER TABLE `bpm_approval_scheme` ADD COLUMN `scene_id` BIGINT COMMENT '关联场景ID' AFTER `biz_type`;
    END IF;
    
    -- 检查并添加索引
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_NAME = 'bpm_approval_scheme' AND INDEX_NAME = 'idx_bpm_approval_scheme_owner') THEN
        ALTER TABLE `bpm_approval_scheme` ADD INDEX `idx_bpm_approval_scheme_owner` (`owner_user_id`);
    END IF;
    
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_NAME = 'bpm_approval_scheme' AND INDEX_NAME = 'idx_bpm_approval_scheme_scene') THEN
...
-- 9. 插入预置模板数据（使用 INSERT IGNORE 避免重复插入）
...
-- 10. 插入站内信通知模板（使用 INSERT IGNORE 避免重复插入）

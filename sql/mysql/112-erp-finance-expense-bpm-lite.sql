/*
 Target: ERP finance expense lightweight BPM integration
...
SET @add_process_instance_id = (
  SELECT IF(
    ...
    'SELECT 1',
    'ALTER TABLE `erp_finance_expense` ADD COLUMN `process_instance_id` varchar(64) DEFAULT NULL COMMENT ''BPM流程实例编号'' AFTER `status`'
  )
);
...
SET @add_idx_process_instance_id = (
  SELECT IF(
    EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'erp_finance_expense'
        AND INDEX_NAME = 'idx_erp_finance_expense_process_instance_id'
    ),
    'SELECT 1',

...
export interface DictDataBatchUpdateReqVO {
  ids: number[]
  fieldKey: string
  mode: string
  value: string
...
export interface DictDataBatchUpdateResultVO {
  successCount: number
  failureCount: number
  updatedIds: number[]
  failedItems: Array<{
...
// 批量修改字典数据
export const batchUpdateDictData = (data: DictDataBatchUpdateReqVO) => {
  return request.put<DictDataBatchUpdateResultVO>({ url: '/system/dict-data/batch-update', data })
}

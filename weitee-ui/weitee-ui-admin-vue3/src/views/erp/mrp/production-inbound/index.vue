<template>
  <div class="min-h-full bg-[var(--erp-slate-50)] p-3 sm:p-4 lg:p-5">
    ...
    <section class="erp-card mb-4">
      <div
        class="flex items-center justify-between border-b border-[var(--erp-slate-100)] px-5 py-3"
      >
      ...
      <el-form ref="queryFormRef" :model="queryParams" label-width="82px" class="px-5 pb-4 pt-4">
        ...
        <div class="flex justify-end gap-2">
          <el-button
            ...
            :loading="listLoading"
            @click="handleQuery"
            v-hasPermi="['erp:production-inbound:query']"
          >
      ...
      <div class="overflow-x-auto">
        ...
        <el-table v-loading="listLoading" :data="list" class="min-w-[980px]" table-layout="fixed">
          <el-table-column label="入库信息" min-width="230" fixed="left">
            <template #default="{ row }">
              <div class="space-y-1 py-1">
                ...
                <div
                  class="truncate text-xs text-[var(--erp-slate-500)]"
          ...
          <el-table-column label="产品 / 仓库" min-width="160">
            <template #default="{ row }">
              <div class="space-y-1">
                <div class="font-mono text-sm text-[var(--erp-slate-800)]"
                  >产品 {{ row.productId || '-' }}</div
                >
                <div
                  class="truncate text-xs text-[var(--erp-slate-500)]"
          ...
          <el-table-column label="数量" min-width="110" align="right">
            <template #default="{ row }"
              ><span class="font-mono">{{ formatCount(row.inboundQty) }}</span></template
            >
          </el-table-column>
          <el-table-column label="单位成本" min-width="130" align="right">
            <template #default="{ row }"
              ><span class="font-mono">{{ formatAmount(row.unitCost) }}</span></template
            >
          </el-table-column>
          ...
          <el-table-column label="状态" min-width="110" align="center">
            <template #default="{ row }">
              <el-tag :type="getStatusTagType(row.status)" effect="light">{{
                getProductionInboundStatusLabel(row.status)
              }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="执行时间" min-width="175">
            <template #default="{ row }">{{
              row.executedTime ? formatDate(row.executedTime, 'YYYY-MM-DD HH:mm:ss') : '-'
            }}</template>
          </el-table-column>
            ...
            <template #default="{ row }">
              <div class="flex items-center justify-center gap-1">
                ...
                <el-button
                  ...
                  link
                  type="success"
                  :disabled="isRowOperating(row.id)"
          ...
          <template #empty>
            <el-empty :description="listErrorMessage || '暂无自制入库单'" :image-size="72">
              <el-button
                v-if="listErrorMessage"
                type="primary"
                plain
    ...
    <el-drawer
      ...
      :with-header="false"
      size="480px"
      class="production-inbound-detail"
    >
      <div v-if="currentRow" class="flex h-full flex-col bg-[var(--erp-slate-50)]">
        <div class="bg-[var(--erp-slate-800)] px-5 py-5 text-white">
          ...
          <div class="mt-2 flex items-center justify-between gap-3">
            <span class="text-sm text-[var(--erp-slate-200)]">{{
              currentRow.productionOrderNo || '-'
            }}</span>
            <el-tag :type="getStatusTagType(currentRow.status)" effect="light">{{
              getProductionInboundStatusLabel(currentRow.status)
            }}</el-tag>
          </div>
        </div>
        <div class="flex-1 overflow-y-auto p-4">
          <section class="erp-card">
            <el-descriptions :column="1" border>
              <el-descriptions-item label="质检单">{{
                currentRow.finishQualityNo || '-'
              ...
              <el-descriptions-item label="入库数量">{{
                formatCount(currentRow.inboundQty)
              }}</el-descriptions-item>
              <el-descriptions-item label="单位成本">{{
                formatAmount(currentRow.unitCost)
              }}</el-descriptions-item>
              <el-descriptions-item label="入库成本">{{
                formatAmount(currentRow.totalCost)
              }}</el-descriptions-item>
              <el-descriptions-item label="执行时间">
                {{
                  currentRow.executedTime
                    ? formatDate(currentRow.executedTime, 'YYYY-MM-DD HH:mm:ss')
                    : '-'
                }}
              </el-descriptions-item>
        ...
        <div
          class="flex justify-end gap-2 border-t border-[var(--erp-slate-200)] bg-white px-4 py-3"
        >
          <el-button @click="detailVisible = false">关闭</el-button>
          <el-button
            v-if="canExecute(currentRow.status)"
            type="primary"
            :disabled="isRowOperating(currentRow.id)"
...
const isRowOperating = (id?: number) => Boolean(id) && operatingId.value === id
...
const refreshAfterOperation = async () => {
  ...
  if (currentRow.value?.id) {
    const refreshedRow = list.value.find((item) => item.id === currentRow.value?.id)
    if (refreshedRow) {
      currentRow.value = refreshedRow
    } else {
...
const handleExecute = async (row: ProductionInboundVO) => {
  if (!row.id || isRowOperating(row.id) || !canExecute(row.status)) return
  try {
    await message.confirm(`确认执行入库单 ${row.no || row.id} 吗？`)
  } catch {
    return
  }
  if (isRowOperating(row.id)) return
...
const handleRevert = async (row: ProductionInboundVO) => {
  if (!row.id || isRowOperating(row.id) || !canRevert(row.status)) return
  try {

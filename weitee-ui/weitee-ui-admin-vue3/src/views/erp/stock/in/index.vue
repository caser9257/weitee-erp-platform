  ...
  <ContentWrap class="stock-in-page__list-card">
    <div class="stock-in-toolbar">
      <div class="stock-in-toolbar__actions">
        ...
        >
          <Icon icon="ep:plus" class="mr-5px" /> 新增入库单
        </el-button>
        <el-button
    ...
    <div class="stock-in-page__footer">
      <div class="stock-in-page__record-count">共 {{ total }} 条记录</div>
      <Pagination
        :total="total"
...
const canSubmitStockIn = checkPermi(['erp:stock-in:submit'])
const canCancelStockInApproval = checkPermi(['erp:stock-in:cancel-approval'])
...
const isApprovalRunning = (row: StockInListRow) =>
  row.status === STOCK_IN_STATUS.PROCESS && !!row.processInstanceId
const canEdit = (row: StockInListRow) =>
  row.status !== STOCK_IN_STATUS.APPROVE && !isApprovalRunning(row)
const canDeleteRow = (row: StockInListRow) =>
  row.status !== STOCK_IN_STATUS.APPROVE && !isApprovalRunning(row)
...
const getCanSubmit = (row: StockInListRow) => {
  return (
    row.status === STOCK_IN_STATUS.REJECT ||
    row.status === STOCK_IN_STATUS.FAILED ||
    (row.status === STOCK_IN_STATUS.DRAFT && !row.processInstanceId) ||
    (row.status === STOCK_IN_STATUS.PROCESS && !row.processInstanceId)
  )
}

const getCanCancelApproval = (row: StockInListRow) => {
  return (
    row.status === STOCK_IN_STATUS.PROCESS &&
...
const getAllActionDescriptors = (row: StockInListRow): StockInActionDescriptor[] => {
  ...
  if (canSubmitStockIn && getCanSubmit(row)) {
    actions.push({
      key: 'submit',
      label:
        row.status === STOCK_IN_STATUS.REJECT || row.status === STOCK_IN_STATUS.FAILED
          ? '重新提交审批'
          : '提交审批'
    })
  }
  if (canCancelStockInApproval && getCanCancelApproval(row)) {
    actions.push({
      key: 'cancelApproval',
      label: '撤回审批',
  ...
  if (canDeleteStockIn && canDeleteRow(row)) {
    actions.push({
      key: 'delete',
      label: '删除',
...
/** 提交审批 */
const handleSubmit = async (row: StockInListRow) => {
  if (!row.id) return
  try {
    await message.confirm(`确定提交审批该入库单吗？`)
    await StockInApi.submitStockIn({ id: row.id })
    await getList()
    const latestRow = list.value.find((item) => item.id === row.id)
    if (latestRow?.status === STOCK_IN_STATUS.FAILED) {
...
/** 撤回审批 */
const handleCancelApproval = async (row: StockInListRow) => {
  ...
  try {
    ...
    await StockInApi.cancelStockInApproval({
      id: row.id,
      reason: value
    })

<template>
  ...
  >
    <template #context>
      <div class="supplier-batch-edit__context">
        <div class="supplier-batch-edit__context-title">供应商</div>
        <div class="supplier-batch-edit__context-count">已选择 {{ selectedCount }} 条</div>
      </div>
    </template>

    <div class="supplier-batch-edit__panel">
      <el-form
        ref="formRef"
        :model="formData"
        :rules="formRules"
        label-position="top"
        class="supplier-batch-edit-form"
      >
...
import {
  SupplierApi,
  type SupplierBatchUpdateResultVO,
  type SupplierVO
} from '@/api/erp/purchase/supplier'
import {
  supplierBatchEditAdapter,
  type SupplierBatchFieldKey
} from './supplierBatchEditAdapter'

defineOptions({ name: 'SupplierBatchEditDrawer' })

const emit = defineEmits<{
  (event: 'success', result: SupplierBatchUpdateResultVO): void
}>()
...
const resolveSuccessMessage = (result: SupplierBatchUpdateResultVO) => {
...
const handleFieldChange = async () => {
  ...
  formData.value = field
    ? supplierBatchEditAdapter.resolveInitialValue(fieldKey, selectedRows.value, getDefaultValue(field.key))
    : ''
  await nextTick()
...
const handleSubmit = async () => {
  ...
  try {
    const fieldKey = supplierBatchEditAdapter.normalizeFieldKey(formData.fieldKey)
    if (!fieldKey) {
      message.warning('请先选择修改字段')
      return
    }
    const result = await SupplierApi.batchUpdateSupplier(
      supplierBatchEditAdapter.buildRequest({
...
const openDrawer = async (payload: { ids: number[]; rows?: SupplierVO[]; fieldKey?: string }) => {
  open({ ids: payload.ids, rows: payload.rows })
  formData.fieldKey = supplierBatchEditAdapter.normalizeFieldKey(payload.fieldKey)
  formData.value = supplierBatchEditAdapter.resolveInitialValue(
    formData.fieldKey,
    selectedRows.value,
...
.supplier-batch-edit__context-title {
  font-size: 18px;
  font-weight: 800;
  line-height: 28px;
}

.supplier-batch-edit__context-count {
  color: #bfdbfe;
  font-size: 12px;
  line-height: 18px;
  font-weight: 600;
...
.supplier-batch-edit__panel {
  padding: 20px;
}

.supplier-batch-edit-form {
  display: grid;
  gap: 18px;
}

  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="1440">
    ...
    <template #footer>
      <el-button
        ...
        type="primary"
        plain
        :disabled="formLoading || !formData.id"
...
const openPrintDialog = () => {
  ...
  printDialogRef.value?.open(formData.value.id)
}

/** 打开【可退货的订单列表】弹窗 */
const purchaseOrderReturnEnableListRef = ref<any>() // 可退货的订单列表 Ref
const openPurchaseOrderReturnEnableList = () => {
  purchaseOrderReturnEnableListRef.value.open()
}

const handlePurchaseOrderChange = (order: PurchaseOrderVO & Record<string, any>) => {
  // 将订单设置到退货单

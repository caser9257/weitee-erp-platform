      ...
      <el-table-column label="操作" align="center" fixed="right" width="220">
        <template #default="scope">
          <div class="flex flex-wrap justify-center gap-x-2 gap-y-1">
            ...
            <el-button
              v-if="canSubmitPurchaseReturn && getCanSubmit(scope.row)"
              link
              @click="handleSubmit(scope.row)"
            >
              {{ scope.row.status === PURCHASE_RETURN_STATUS.REJECT ? '重新提交审批' : '提交审批' }}
            </el-button>
            <el-button
              v-if="canCancelPurchaseReturnApproval && getCanCancelApproval(scope.row)"
              link
              type="warning"
              :disabled="isCancelingApproval(scope.row.id)"
              :loading="isCancelingApproval(scope.row.id)"
...
const canSubmitPurchaseReturn = checkPermi(['erp:purchase-return:submit'])
const canCancelPurchaseReturnApproval = checkPermi(['erp:purchase-return:cancel-approval'])
...
const cancelApprovalIds = ref<number[]>([]) // 撤回审批中的 ID 列表
...
const getCanSubmit = (row: PurchaseReturnVO) => {
  return (
    row.status === PURCHASE_RETURN_STATUS.REJECT ||
    (row.status === PURCHASE_RETURN_STATUS.DRAFT && !row.processInstanceId) ||
    (row.status === PURCHASE_RETURN_STATUS.PROCESS && !row.processInstanceId)
  )
}

const getCanCancelApproval = (row: PurchaseReturnVO) => {
  return (
    row.status === PURCHASE_RETURN_STATUS.PROCESS &&

<template>
  <el-drawer
    ...
    title="研发BOM详情"
    size="960px"
    class="rd-bom-detail-drawer"
    body-class="rd-bom-detail-drawer__body"
    destroy-on-close
  >
    <div v-loading="detailLoading" class="rd-bom-detail-drawer__content">
      <template v-if="detailData">
        <section class="rd-bom-detail-drawer__base-info">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="最近发布时间">
              {{ detailData.lastPublishedTime ? formatDate(detailData.lastPublishedTime) : '-' }}
            </el-descriptions-item>
        ...
        <section class="rd-bom-detail-drawer__material-section">
          <el-divider content-position="left">设计物料</el-divider>

          <div class="rd-bom-detail-drawer__table-scroll">
            <el-table
              :data="detailData.items || []"
              border
              height="100%"
              class="rd-bom-detail-drawer__table"
            >
              ...
              <el-table-column label="物料" prop="materialName" min-width="180" fixed="left" />
              <el-table-column
                label="产品型号"
        ...
        <section class="rd-bom-detail-drawer__history-section">
          ...
          <div v-loading="chainLoading" class="min-h-80px">
            ...
            <div v-else-if="!chainLoading" class="py-20px text-center text-13px text-slate-400"
              >暂无其他版本</div
            >
          </div>
          ...
          <div v-loading="changeLogLoading" class="min-h-120px">
                ...
                <div class="rounded border border-slate-100 bg-slate-50 px-12px py-10px">
                  <div class="mb-6px flex items-center gap-8px">
                    <el-tag size="small" effect="plain">{{
                      CHANGE_TYPE_META[log.changeType]?.label || log.changeType
                    }}</el-tag>
                    <span v-if="log.creator" class="text-12px text-slate-500"
                      >操作人 {{ log.creator }}</span
                    >
                  </div>
            ...
            <div v-else-if="!changeLogLoading" class="py-32px text-center text-13px text-slate-400"
              >暂无变更记录</div
            >
          </div>
        </section>
...
import {
  STATUS_META,
  canChange as canChangeFn,
  canVoid as canVoidFn,
  canUnvoid as canUnvoidFn
} from './rdBomStatus'
...
const canChange = computed(() => canChangeFn(detailData.value))
const canVoid = computed(() => canVoidFn(detailData.value))
const canUnvoid = computed(() => canUnvoidFn(detailData.value))
...
:deep(.rd-bom-detail-drawer__body) {
  display: flex;
  flex: 1;
  flex-direction: column;
  min-height: 0;
...
.rd-bom-detail-drawer__base-info {
  flex-shrink: 0;
}

.rd-bom-detail-drawer__material-section {
  display: flex;
  flex: 1 1 320px;
  flex-direction: column;
  min-height: 0;
...
.rd-bom-detail-drawer__table-scroll {
  flex: 1;
  min-height: 0;
  overflow: auto;
}

.rd-bom-detail-drawer__table {
  min-width: 0;
}

.rd-bom-detail-drawer__history-section {
  flex: 0 1 32%;
  min-height: 0;
  overflow: auto;
}

.rd-bom-detail-drawer__material-section :deep(.el-divider),
.rd-bom-detail-drawer__history-section :deep(.el-divider) {
  flex-shrink: 0;
}
</style>

...
const handleAddSaleOut = (rows: SaleOutVO[]) => {
  rows.forEach((row) => {
    const saleOut = row as SaleOutVO & Record<string, any>
    formData.value.push({
      bizId: saleOut.id,
      bizType: ErpBizType.SALE_OUT,
      bizNo: saleOut.no,
      totalPrice: saleOut.totalPrice,
      receiptedPrice: saleOut.receiptPrice,
      receiptPrice: saleOut.totalPrice - saleOut.receiptPrice
    })
  })
}
...
const handleAddSaleReturn = (rows: SaleReturnVO[]) => {
  rows.forEach((row) => {
    const saleReturn = row as SaleReturnVO & Record<string, any>
    formData.value.push({
      bizId: saleReturn.id,
      bizType: ErpBizType.SALE_RETURN,
      bizNo: saleReturn.no,
      totalPrice: -saleReturn.totalPrice,
      receiptedPrice: -saleReturn.refundPrice,
      receiptPrice: -saleReturn.totalPrice + saleReturn.refundPrice
    })
  })
}

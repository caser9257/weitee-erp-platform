  ...
  <Dialog title="提交审批" v-model="dialogVisible" width="960" @closed="handleClosed">
    <div v-loading="loading">
      <el-alert
        ...
        :closable="false"
        show-icon
        title="提交后将进入 BPM 审批流，审批中不可编辑，请确认付款信息无误后再发起。"
      />
      <el-descriptions :column="2" border class="mb-16px">
        ...
        <el-descriptions-item label="实际付款">
          {{ erpPriceInputFormatter(currentPayment?.paymentPrice) }}
...
const buildProcessVariables = (payment: FinancePaymentVO) => {
  return {
    ...
    creatorId: payment.creator && /^\d+$/.test(payment.creator) ? Number(payment.creator) : undefined
  }
}
...
const open = async (payment: FinancePaymentVO) => {
  ...
  try {
    ...
    if (!definition?.id) {
      message.error('付款单审批流程未配置，请先发布 BPM 流程定义')

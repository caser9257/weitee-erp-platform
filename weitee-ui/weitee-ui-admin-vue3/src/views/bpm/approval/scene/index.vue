<template>
  <div class="approval-page approval-scene-page">
    <!-- 页头卡片 -->
    <ContentWrap class="approval-page__header-card">
      <div class="approval-page__page-header">
        <div class="approval-page__page-title">审批场景</div>
      ...
      <el-form ref="queryFormRef" :model="queryParams" label-width="84px" class="approval-page__query-form" @submit.prevent>
        <div class="approval-page__query-grid">
          <el-form-item label="场景编码" prop="sceneCode">
            <el-input v-model="queryParams.sceneCode" placeholder="请输入场景编码" clearable class="!w-full" @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item label="场景名称" prop="name">
            <el-input v-model="queryParams.name" placeholder="请输入场景名称" clearable class="!w-full" @keyup.enter="handleQuery" />
          </el-form-item>
          ...
          <el-form-item label="状态" prop="status">
            <el-select v-model="queryParams.status" placeholder="请选择状态" clearable class="!w-full">
              <el-option v-for="item in SCENE_STATUS_OPTIONS" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </div>
    ...
    <ContentWrap class="approval-page__table-card">
      <div class="approval-page__toolbar">
        <div class="approval-page__toolbar-main">
          <div class="approval-page__section-title">场景列表</div>
      ...
      <div class="approval-page__table-wrap">
        <el-table v-loading="loading" :data="list" row-key="id" stripe :show-overflow-tooltip="false" class="approval-page__table">
          <el-table-column min-width="260">
            <template #header>
              <span class="approval-page__column-header">
                <Icon icon="ep:coordinate" class="approval-page__column-icon" />
                审批场景
              </span>
            </template>
            <template #default="{ row }">
              <div class="approval-page__primary-cell">
                ...
                <span class="approval-page__muted-text approval-page__mono">{{ row.sceneCode || '-' }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column min-width="220">
            ...
            <template #default="{ row }">
              <div class="approval-page__primary-cell">
                ...
                <span class="approval-page__muted-text">
                  {{ row.bizType || '-' }} · {{ row.actionCode || '-' }}
                </span>
              </div>
            </template>
          </el-table-column>
          <el-table-column min-width="180" label="绑定方案">
            <template #default="{ row }">
              <span v-if="row.activeSchemeId" class="approval-page__scheme-tag">
                <Icon icon="ep:document-checked" class="approval-page__scheme-tag-icon" />
                {{ row.activeSchemeName || `方案#${row.activeSchemeId}` }}
              </span>
              <span v-else class="approval-page__muted-text">未绑定</span>
            </template>
          </el-table-column>
          ...
          <el-table-column min-width="210" label="操作" align="center" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="openForm('update', row.id)" v-hasPermi="['bpm:approval-scene:update']">
                编辑
              </el-button>
              <el-button
                v-if="row.status === 1"
                ...
                v-hasPermi="['bpm:approval-scene:update']"
              >
                禁用
              </el-button>
              <el-button
              ...
              <el-button link type="danger" @click="handleDelete(row.id)" v-hasPermi="['bpm:approval-scene:delete']">
                删除
              </el-button>
            </template>
          </el-table-column>
...
import { ApprovalSceneApi, BpmApprovalSceneVO } from '@/api/bpm/approval/scene'
...
const list = ref<BpmApprovalSceneVO[]>([])
const total = ref(0)
...
const summaryCards = ref([
  { label: '全部场景', value: 0, icon: 'ep:coordinate', toneClass: 'approval-page__metric--blue' },
  { label: '已启用', value: 0, icon: 'ep:circle-check', toneClass: 'approval-page__metric--green' },
  { label: '已禁用', value: 0, icon: 'ep:circle-close', toneClass: 'approval-page__metric--muted' }
])
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ApprovalSceneApi.getScenePage(queryParams)
...
const refreshSummary = (rows: BpmApprovalSceneVO[]) => {
  summaryCards.value[0].value = total.value
  summaryCards.value[1].value = rows.filter((item) => item.status === 1).length
  summaryCards.value[2].value = rows.filter((item) => item.status === 0).length
}
...
/** 启用/禁用 */
const handleToggleStatus = async (row: BpmApprovalSceneVO, status: number) => {
  try {
    await message.confirm(status === 1 ? `确定启用「${row.name}」吗？` : `确定禁用「${row.name}」吗？`)
    if (status === 1) {
      await ApprovalSceneApi.enableScene(row.id!)
    } else {
      await ApprovalSceneApi.disableScene(row.id!)
    }
...
const handleDelete = async (id: number) => {
  try {
    ...
    await ApprovalSceneApi.deleteScene(id)

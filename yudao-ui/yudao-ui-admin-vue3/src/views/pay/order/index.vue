  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="支付金额" align="center" prop="price" width="100">
        <template #default="scope"> ￥{{ ((Number(scope.row.price) || 0) / 100).toFixed(2) }} </template>
      </el-table-column>
      <el-table-column label="退款金额" align="center" prop="refundPrice" width="100">
        <template #default="scope">
          ￥{{ ((Number(scope.row.refundPrice) || 0) / 100).toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="手续金额" align="center" prop="channelFeePrice" width="100">
        <template #default="scope">
          ￥{{ ((Number(scope.row.channelFeePrice) || 0) / 100).toFixed(2) }}
        </template>
      </el-table-column>

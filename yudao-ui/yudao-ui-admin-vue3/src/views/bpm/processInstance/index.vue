    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="180">
        <template #default="scope">
          ...
          <el-button
            v-else-if="canRestartProcessInstance(scope.row.status)"
            link
            type="primary"
...
} from './processInstanceRestart.helpers'
...
const handleCreate = async (row?: ProcessInstanceVO) => {
  if (row?.id) {
    ...
    const restartTargetOptions = {
      processDefinitionKey: processDefinitionDetail.key,
      formType: processDefinitionDetail.formType,
      formCustomCreatePath: processDefinitionDetail.formCustomCreatePath,
      businessKey: row.businessKey,
      processInstanceId: row.id
    }
    if (isSaleOrderRestartTarget(restartTargetOptions)) {
      const saleOrderId = Number(row.businessKey)
      if (!saleOrderId || Number.isNaN(saleOrderId)) {
        message.warning('未找到对应的销售订单，无法重新发起')
        return
      }
      ...
      if (!canRestartSaleOrderBusinessRecord(saleOrder)) {
        ...
        if (saleOrder.status === 10 && saleOrder.processInstanceId) {
          message.warning('该销售订单已有新的审批流程在进行中，不能基于历史流程重新发起')
          return
        }
        message.warning('该销售订单当前状态不支持重新发起')
        return
      }
    }
    const restartTarget = resolveProcessInstanceRestartTarget({
      ...restartTargetOptions
    })
    if (!restartTarget) {
      message.warning('当前流程暂不支持重新发起')
      return
    }
    await router.push(restartTarget)
  }
}

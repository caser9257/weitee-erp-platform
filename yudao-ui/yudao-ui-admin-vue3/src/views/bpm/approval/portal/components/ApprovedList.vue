<template>
  <div class="approved-list">
    <el-table v-loading="loading" :data="list" :stripe="true">
      ...
      <el-table-column label="处理节点" prop="taskName" width="150" />
      <el-table-column label="处理结果" prop="status" width="100">
        <template #default="{ row }">
...
defineOptions({ name: 'ApprovedList' })
...
const getStatusLabel = (status: number) => {
  const map: Record<number, string> = { 2: '通过', 3: '拒绝', 4: '撤回' }
...
const getList = async () => {
  ...
  try {
    ...
    message.error(getErrorMessage(error) || '我已审批列表加载失败，请稍后重试')
  } finally {
    loading.value = false
  }
}

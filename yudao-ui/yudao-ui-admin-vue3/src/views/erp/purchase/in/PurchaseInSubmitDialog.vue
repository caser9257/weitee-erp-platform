  ...
  <Dialog title="提交审批" v-model="dialogVisible" width="960">
    <div v-loading="loading">
      <el-alert
        ...
        :closable="false"
        show-icon
        title="提交后将进入 BPM 审批流，审批中不可编辑，请确认采购入库信息无误后再发起。"
      />
      <el-descriptions :column="2" border class="mb-16px">
        <el-descriptions-item label="入库单号">{{ currentPurchaseIn?.no || '-' }}</el-descriptions-item>
        <el-descriptions-item label="采购订单">{{ currentPurchaseIn?.orderNo || currentPurchaseIn?.orderId || '-' }}</el-descriptions-item>
        <el-descriptions-item label="供应商编号">{{ currentPurchaseIn?.supplierId || '-' }}</el-descriptions-item>
        <el-descriptions-item label="应付金额">
          {{ erpPriceInputFormatter(currentPurchaseIn?.totalPrice) }}
...
defineOptions({ name: 'PurchaseInSubmitDialog' })

const PROCESS_DEFINITION_KEY = 'erp_purchase_in'
...
const currentPurchaseIn = ref<PurchaseInVO>()
...
const resetState = () => {
  ...
  currentPurchaseIn.value = undefined
}

const buildProcessVariables = (purchaseIn: PurchaseInVO) => {
  return {
    purchaseInId: purchaseIn.id,
    purchaseInNo: purchaseIn.no,
    totalPrice: purchaseIn.totalPrice,
    supplierId: purchaseIn.supplierId,
    purchaseOrderId: purchaseIn.orderId,
    creatorId:
      purchaseIn.creator && /^\d+$/.test(purchaseIn.creator) ? Number(purchaseIn.creator) : undefined
  }
}
...
const getApprovalDetail = async () => {
  if (!processDefinitionId.value || !currentPurchaseIn.value) {
    return
  }
  const data = await ProcessInstanceApi.getApprovalDetail({
    processDefinitionId: processDefinitionId.value,
    activityId: NodeId.START_USER_NODE_ID,
    processVariablesStr: JSON.stringify(buildProcessVariables(currentPurchaseIn.value))
  })
...
const open = async (purchaseIn: PurchaseInVO) => {
  dialogVisible.value = true
  loading.value = true
  resetState()
  currentPurchaseIn.value = { ...purchaseIn }
  try {
    const definition = await DefinitionApi.getProcessDefinition(undefined, PROCESS_DEFINITION_KEY)
    if (!definition?.id) {
      message.error('采购入库审批流程未配置，请先发布 BPM 流程定义')
...
const submit = async () => {
  ...
  try {
    await PurchaseInApi.submitPurchaseIn({
      id: currentPurchaseIn.value.id!,

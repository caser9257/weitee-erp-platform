      ...
      <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="px-4 py-4 sm:px-5">
          ...
          >
            ...
            <el-table-column label="关联台账" min-width="200">
              <template #default="{ row }">
                <div v-if="row.statementId" class="space-y-1">
                  <div class="flex items-center gap-1.5">
                    <span class="shrink-0 rounded-full bg-blue-50 px-1.5 py-0.5 text-[11px] font-medium text-blue-600 whitespace-nowrap">
                      台账
                    </span>
                    <span class="min-w-0 flex-1 truncate font-mono text-slate-800" :title="row.statementNo || '-'">
                      {{ row.statementNo || '-' }}
                    </span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    ...
                    <span class="min-w-0 flex-1 truncate font-mono text-slate-600">
                      {{ formatNumber(row.statementPaidAmount) }} / {{ formatNumber(row.statementAmount) }}
                    </span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    <el-tag :type="resolveStatementStatusTagType(row.statementStatus)" size="small" effect="light">
                      {{ row.statementStatusName || '-' }}
                    </el-tag>
                  </div>
                </div>
...
const resolveStatementStatusTagType = (status?: number) => {
  if (status === 10) return 'warning' // 待付款
  if (status === 20) return 'success' // 已付款
  if (status === 30) return 'info' // 已关闭
  return 'info'
}

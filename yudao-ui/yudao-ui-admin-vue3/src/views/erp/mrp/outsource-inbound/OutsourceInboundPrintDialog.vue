  ...
  >
    ...
    <template v-if="detail">
      ...
      <div class="finance-shell__section">
        ...
        <div
          v-if="reconciliationRecords.length"
          class="grid gap-3"
        >
          <div
            v-for="(item, index) in reconciliationRecords"
            :key="`${item.paymentId || 'payment'}-${index}`"
            class="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm"
          >
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <div>
                <div class="text-xs text-slate-500">付款单号</div>
                <div class="mt-1 font-mono text-slate-800">{{ item.paymentNo || '-' }}</div>
              </div>
              <div>
                <div class="text-xs text-slate-500">核销金额</div>
                <div class="mt-1 font-mono text-slate-800">{{ formatNumber(item.allocateAmount) }}</div>
              </div>
              <div>
                <div class="text-xs text-slate-500">核销时间</div>
                <div class="mt-1 text-slate-800">{{ formatDateValue(item.paymentTime) }}</div>
              </div>
              <div>
                <div class="text-xs text-slate-500">操作人</div>
                <div class="mt-1 text-slate-800">{{ item.operatorName || '-' }}</div>
              </div>
            </div>
            <div class="mt-3 grid gap-3 md:grid-cols-[180px,1fr]">
              <div>
                <div class="text-xs text-slate-500">核销状态</div>
                <div class="mt-1">
                  <el-tag effect="light" :type="getAllocateStatusTagType(item.status)">
                    {{ item.statusName || '-' }}
                  </el-tag>
                </div>
              </div>
              <div>
                ...
                <div class="mt-1 text-slate-700">{{ item.remark || '-' }}</div>
              </div>
            </div>
          </div>
        </div>
...
const reconciliationRecords = ref<OutsourceInboundPrintDataVO['reconciliationRecords']>([])
...
const getAllocateStatusTagType = (status?: number) => {
  if (status === 20) return 'success'
  if (status === 30) return 'info'
  return 'warning'
}
...
const loadPrintData = async (id: number) => {
  ...
  try {
    ...
    reconciliationRecords.value = data.reconciliationRecords || []
    loadErrorMessage.value = ''
  } catch {
    detail.value = undefined

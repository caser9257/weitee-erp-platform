<template>
  <div class="finance-shell finance-shell__stack finish-quality-page">
    <ContentWrap class="finance-shell__filter-card">
      <div class="finance-shell__section-head">
        <div>
          <div class="finance-shell__section-title">完工质检查询</div>
          <div class="finance-shell__section-subtitle">
            支持按质检单、生产工单和状态筛选，并可直接穿透查看工单成本。
          </div>
        </div>
      </div>
      <el-form
    ...
    <ContentWrap class="finance-shell__table-card">
      <div class="finance-shell__toolbar">
        <div class="finance-shell__table-toolbar-main">
          <div class="finance-shell__section-title">质检列表</div>
        ...
        >
          ...
          <el-table-column label="质检状态" width="120" align="center">
            <template #default="{ row }">
              <el-tag :type="resolveFinishQualityStatusTagType(row.status)">
                {{ resolveFinishQualityStatusLabel(row.status) }}
              </el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" fixed="right" width="320" align="center">
            <template #default="{ row }">
              <div class="finance-shell__row-actions">
                ...
                <el-button
                  v-if="hasInboundQueryPermission && canViewProductionInbound(row)"
                  link
                  type="primary"
                  @click="openProductionInboundPage(row.id!, row.productionOrderId)"
                >
                  查看入库单
                </el-button>
                <el-button
        ...
        <template v-else>
          <ContentWrap class="!mb-12px">
            <el-descriptions :column="2" border>
              ...
              <el-descriptions-item label="销售单ID">
                {{ detailForm.sourceOrderId || '-' }}
              </el-descriptions-item>
              ...
              <el-descriptions-item label="状态">
                <el-tag :type="resolveFinishQualityStatusTagType(detailForm.status)">
                  {{ resolveFinishQualityStatusLabel(detailForm.status) }}
                </el-tag>
              </el-descriptions-item>
              <el-descriptions-item label="质检时间">
                {{
                  detailForm.checkTime
                    ? formatDate(detailForm.checkTime as Date, 'YYYY-MM-DD HH:mm:ss')
                    : '-'
                }}
              </el-descriptions-item>
              <el-descriptions-item label="创建时间">
                {{
                  detailForm.createTime
                    ? formatDate(detailForm.createTime as Date, 'YYYY-MM-DD HH:mm:ss')
                    : '-'
                }}
              </el-descriptions-item>
            </el-descriptions>
...
const hasInboundQueryPermission = checkPermi(['erp:production-inbound:query'])
...
const dialogTitle = computed(() =>
  dialogMode.value === 'submit' ? '处理成品质检' : '查看成品质检'
)
...
const sumMatchesReportQty = computed(() => {
  return (
    roundCount(Number(detailForm.qualifiedQty || 0) + Number(detailForm.unqualifiedQty || 0)) ===
...
const costDetailHeader = computed(() => getFinishQualityCostDetailHeader(costDetailOrderNo.value))
...
const openCostDetailDrawer = (productionOrderId?: number, productionOrderNo?: string) => {
  ...
  costDetailOrderNo.value = productionOrderNo || ''
  costDetailDrawerVisible.value = true
}

const openProductionInboundPage = (finishQualityId: number, productionOrderId?: number) => {
  router.push({
    path: '/mrp/production-inbound',
    query: {
      finishQualityId: String(finishQualityId),
      ...(productionOrderId ? { productionOrderId: String(productionOrderId) } : {})
    }
  })
}

const handleQualifiedChange = () => {
  const next = syncSubmitCounts(
    {
      reportQty: Number(detailForm.reportQty || 0),
      qualifiedQty: Number(detailForm.qualifiedQty || 0),
      unqualifiedQty: Number(detailForm.unqualifiedQty || 0)
    },
    'qualified'
  )
  detailForm.qualifiedQty = next.qualifiedQty
  detailForm.unqualifiedQty = next.unqualifiedQty
}

const handleUnqualifiedChange = () => {
  const next = syncSubmitCounts(
    {
      reportQty: Number(detailForm.reportQty || 0),
      qualifiedQty: Number(detailForm.qualifiedQty || 0),
...
watch(costDetailDrawerVisible, (visible) => {
  if (!visible) {
    ...
    costDetailOrderNo.value = ''
  }
})
</script>

<style scoped>
@import '../../finance/shared/readOnlyPage.css';

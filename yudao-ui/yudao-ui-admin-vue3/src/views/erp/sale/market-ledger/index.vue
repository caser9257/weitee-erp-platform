<template>
  <div class="market-ledger-page">
    ...
    <ContentWrap class="market-ledger-page__header">
      <div class="page-header">
        <div class="page-header__main">
          ...
          <div class="page-header__desc">项目级聚合视图，覆盖合同、订单、收款、发货、开票全链路</div>
        </div>
      ...
      <el-form ref="queryFormRef" :model="queryParams" label-position="top" class="query-form">
        <div class="query-form__grid query-form__grid--primary">
          ...
          <el-form-item label="客户名称" prop="customerName">
            <el-input v-model="queryParams.customerName" clearable placeholder="请输入客户名称" @keyup.enter="loadData" />
          </el-form-item>
          <el-form-item label="销售员" prop="saleUserId">
            <el-select v-model="queryParams.saleUserId" clearable filterable placeholder="请选择销售员">
        ...
        <transition name="query-collapse">
          <div v-if="advancedSearchVisible" class="query-form__grid query-form__grid--advanced">
            ...
            <el-form-item label="验收状态" prop="acceptanceStatus">
              <el-select v-model="queryParams.acceptanceStatus" clearable placeholder="请选择验收状态">
    ...
    <ContentWrap class="market-ledger-page__table">
      ...
      <el-table :data="list" v-loading="loading" stripe class="market-ledger-table" @row-click="openDrawer">
          ...
          <template #default="{ row }">
            <div class="progress-cell">
              <div class="progress-cell__bar">
                <div class="progress-cell__fill progress-cell__fill--success" :style="{ width: `${row.receiptProgress || 0}%` }"></div>
              </div>
        ...
        <el-table-column label="发货状态" min-width="160">
          <template #default="{ row }">
            <div class="status-cell">
              <el-tag :type="releaseStatusType(row.shipmentReleaseStatus)" size="small" effect="light">
                {{ formatReleaseStatus(row.shipmentReleaseStatus) }}
              </el-tag>
              <div class="status-cell__meta" v-if="row.shipmentReleaseReason">{{ row.shipmentReleaseReason }}</div>
            </div>
          </template>
        </el-table-column>
    ...
    <!-- 详情 Drawer -->
    <el-drawer v-model="drawerVisible" :size="480" :with-header="false" class="market-ledger-drawer">
      <div class="drawer-context">
        ...
        <div class="drawer-context__meta">
          <el-tag :type="lifecycleStageType(currentRow?.lifecycleStage)" size="small" effect="light">
        ...
        <div class="drawer-card__content">
          <div class="info-grid">
            <div class="info-item">
              ...
              <div class="info-item__value">{{ formatReleaseRule(currentRow?.shipmentReleaseRule) }}</div>
            </div>
            <div class="info-item">
              <div class="info-item__label">开票触发</div>
              <div class="info-item__value">{{ formatInvoiceTrigger(currentRow?.invoiceTrigger) }}</div>
            </div>
            <div class="info-item">
              <div class="info-item__label">收款规则</div>
              <div class="info-item__value">{{ formatCollectionRule(currentRow?.collectionRule) }}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="progress-list">
            <div class="progress-item">
              ...
              <div class="progress-item__bar">
                <div class="progress-item__fill progress-item__fill--success" :style="{ width: `${currentRow?.receiptProgress || 0}%` }"></div>
              </div>
            ...
            <div class="progress-item">
              ...
              <div class="progress-item__bar">
                <div class="progress-item__fill progress-item__fill--primary" :style="{ width: `${currentRow?.shipmentProgress || 0}%` }"></div>
              </div>
...
import { MarketLedgerApi, type MarketLedgerVO, type MarketLedgerStatsVO } from '@/api/erp/sale/market-ledger'
...
const formatInvoiceStatus = (status?: string) => {
  const map: Record<string, string> = { NOT_INVOICED: '未开票', PARTIAL_INVOICED: '部分开票', FULLY_INVOICED: '全额开票' }
  return map[status || ''] || status || '--'
}
...
const formatReleaseRule = (rule?: string) => {
  const map: Record<string, string> = { SIGN_AND_SHIP: '签约即发', AFTER_PAYMENT: '到账后发', AFTER_PREPAYMENT: '预付款后发', FINANCE_APPROVAL: '财务审核后发' }
  return map[rule || ''] || rule || '--'
}

const formatInvoiceTrigger = (trigger?: string) => {
  const map: Record<string, string> = {
    PREPAYMENT_FULL: '预付款全额开票', PREPAYMENT_PARTIAL: '预付款部分开票', PREPAYMENT_ONLY: '仅预付款开票',
    AFTER_SHIPMENT: '发货后开票', AFTER_DELIVERY_RECEIPT: '交付收款后开票', MANUAL: '手工决定'
  }
...
.market-ledger-page {
  ...
  .progress-cell__fill--success { background: var(--erp-success-500); }
  .progress-cell__fill--primary { background: var(--erp-primary-500); }
  ...
  .market-ledger-drawer {
    ...
    .progress-item__fill--success { background: var(--erp-success-500); }
    .progress-item__fill--primary { background: var(--erp-primary-500); }

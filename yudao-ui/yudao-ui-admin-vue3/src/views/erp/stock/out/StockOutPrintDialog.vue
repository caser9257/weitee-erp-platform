<template>
  <PrintShell
    ...
    title="其他出库打印"
    print-area-id="stockOutPrintArea"
    :loading="loading"
    @closed="handleClosed"
  >
    <template #context>
...
import { StockOutApi, type StockOutVO } from '@/api/erp/stock/out'
...
defineOptions({ name: 'StockOutPrintDialog' })
...
const summaryItems = computed(() => [
  { label: '客户', value: detail.value?.customerName || '-' },
  { label: '出库金额', value: formatMoney(detail.value?.totalPrice) },
  { label: '出库产品', value: detail.value?.productNames || '-' }
])
...
const loadPrintData = async (id: number) => {
  ...
  resetState()
  try {
    const data = await StockOutApi.getStockOutPrintData(id)

<template>
  <div class="finance-asset-page">
      ...
      >
        ...
        <div class="finance-asset-page__source-summary">
          ...
          <div class="finance-asset-page__source-summary-item finance-asset-page__source-summary-item--full finance-asset-page__source-summary-item--actions">
            <el-button
              v-if="canJumpToSourceWithPermission(candidateConfirmForm.sourceType, candidateConfirmForm.sourceBizId)"
              link
              type="primary"
              :disabled="navigatingSource"
          ...
          <div class="finance-asset-page__detail-block">
            ...
            <div class="finance-asset-page__detail-grid">
              ...
              <div class="finance-asset-page__detail-grid-span">
                <span>来源操作</span>
                <strong class="finance-asset-page__detail-actions">
                  <el-button
                    v-if="canOpenSourceTraceEntry"
                    link
                    type="primary"
                    @click="openSourceDetailDialog"
                  >
                    来源追溯
                  </el-button>
                  <span v-if="!canOpenSourceTraceEntry">
                    暂无来源详情
                  </span>
                </strong>
              </div>
    ...
    <Dialog
      v-model="sourceDetailDialogOpen"
      :title="sourceDetailDialogTitle"
      width="920px"
      scroll
      maxHeight="78vh"
      @closed="resetSourceDetailDialog"
    >
      <template v-if="hasPurchaseSourceDetail && assetTrace?.purchaseIn">
        <div class="finance-asset-page__context-card finance-asset-page__context-card--dialog">
          <div class="finance-asset-page__context-main">
            <div class="finance-asset-page__context-eyebrow">来源追溯</div>
            <div class="finance-asset-page__context-title">{{ assetTrace.purchaseIn.no || '-' }}</div>
            <div class="finance-asset-page__context-meta">
              <span>{{ getSourceTypeLabel(assetDetail?.sourceType) }}</span>
              <span>{{ assetTrace.purchaseIn.supplierName || '未维护供应商' }}</span>
              <span>{{ formatDateValue(assetTrace.purchaseIn.inTime) }}</span>
            </div>
          </div>
          <div class="finance-asset-page__context-side">
            <div class="finance-asset-page__context-stat">
              <span>入库总金额</span>
              <strong>{{ formatAmount(assetTrace.purchaseIn.totalPrice) }}</strong>
            </div>
            <div class="finance-asset-page__context-stat">
              <span>已付金额</span>
              <strong>{{ formatAmount(assetTrace.purchaseIn.paymentPrice) }}</strong>
            </div>
          </div>
        </div>

        <div class="finance-asset-page__source-summary finance-asset-page__source-summary--dialog">
          <div class="finance-asset-page__source-summary-item">
            <span>来源单号</span>
            <strong class="finance-asset-page__mono">{{ assetTrace.purchaseIn.no || '-' }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>来源类型</span>
            <strong>{{ getSourceTypeLabel(assetDetail?.sourceType) }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>入库日期</span>
            <strong>{{ formatDateValue(assetTrace.purchaseIn.inTime) }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>供应商</span>
            <strong>{{ assetTrace.purchaseIn.supplierName || '-' }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item finance-asset-page__source-summary-item--full">
            <span>产品摘要</span>
            <strong>{{ assetTrace.purchaseIn.productNames || '-' }}</strong>
          </div>
        </div>

        <div
          v-if="purchaseMatchedItems.length"
          class="finance-asset-page__detail-block finance-asset-page__detail-block--dialog"
        >
          <div class="finance-asset-page__detail-block-title">命中明细</div>
          <div class="finance-asset-page__trace-list">
            <div
              v-for="(item, index) in purchaseMatchedItems"
              :key="item.id || `purchase-matched-${index}`"
              class="finance-asset-page__trace-item finance-asset-page__trace-item--matched"
            >
              <div class="finance-asset-page__trace-item-head">
                <strong>{{ item.productName || '未命名产品' }}</strong>
                <el-tag size="small" effect="light" round type="success">精确命中</el-tag>
              </div>
              <div class="finance-asset-page__trace-item-grid">
                <div><span>数量</span><strong>{{ formatQuantityValue(item.count) }}</strong></div>
                <div><span>金额</span><strong>{{ formatAmount(item.totalPrice) }}</strong></div>
                <div class="finance-asset-page__detail-grid-span">
                  <span>备注</span>
        ...
        <div class="finance-asset-page__detail-block finance-asset-page__detail-block--dialog">
          <div class="finance-asset-page__detail-block-title">整单明细</div>
          <div v-if="assetTrace.purchaseIn.items?.length" class="finance-asset-page__trace-list">
            <div
              v-for="(item, index) in assetTrace.purchaseIn.items"
              :key="item.id || `purchase-item-${index}`"
              :class="['finance-asset-page__trace-item', { 'finance-asset-page__trace-item--matched': item.matched }]"
            >
              <div class="finance-asset-page__trace-item-head">
                <strong>{{ item.productName || '未命名产品' }}</strong>
                <el-tag v-if="item.matched" size="small" effect="light" round type="success">精确命中</el-tag>
              </div>
              <div class="finance-asset-page__trace-item-grid">
                <div><span>数量</span><strong>{{ formatQuantityValue(item.count) }}</strong></div>
                <div><span>金额</span><strong>{{ formatAmount(item.totalPrice) }}</strong></div>
          ...
          <el-empty v-else description="暂无来源详情" />
        </div>
      </template>

      <template v-else-if="hasExpenseSourceDetail && assetTrace?.expense">
        <div class="finance-asset-page__context-card finance-asset-page__context-card--dialog">
          <div class="finance-asset-page__context-main">
            <div class="finance-asset-page__context-eyebrow">来源追溯</div>
            <div class="finance-asset-page__context-title">{{ assetTrace.expense.no || '-' }}</div>
            <div class="finance-asset-page__context-meta">
              <span>{{ getSourceTypeLabel(assetDetail?.sourceType) }}</span>
              <span>{{ assetTrace.expense.expenseTypeName || '未维护费用类型' }}</span>
              <span>{{ formatDateValue(assetTrace.expense.expenseTime) }}</span>
            </div>
          </div>
          <div class="finance-asset-page__context-side">
            <div class="finance-asset-page__context-stat">
              <span>报销金额</span>
              <strong>{{ formatAmount(assetTrace.expense.expensePrice) }}</strong>
            </div>
            <div class="finance-asset-page__context-stat">
              <span>已付金额</span>
              <strong>{{ formatAmount(assetTrace.expense.paidPrice) }}</strong>
            </div>
          </div>
        </div>
        ...
        <div class="finance-asset-page__source-summary finance-asset-page__source-summary--dialog">
          <div class="finance-asset-page__source-summary-item">
            <span>来源单号</span>
            <strong class="finance-asset-page__mono">{{ assetTrace.expense.no || '-' }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>来源类型</span>
            <strong>{{ getSourceTypeLabel(assetDetail?.sourceType) }}</strong>
          ...
          <div class="finance-asset-page__source-summary-item">
            <span>报销日期</span>
            <strong>{{ formatDateValue(assetTrace.expense.expenseTime) }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>费用类型</span>
            <strong>{{ assetTrace.expense.expenseTypeName || '-' }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>部门</span>
            <strong>{{ assetTrace.expense.deptName || '-' }}</strong>
          </div>
          <div class="finance-asset-page__source-summary-item">
            <span>项目</span>
            <strong>{{ assetTrace.expense.projectName || '-' }}</strong>
          </div>
        </div>

        <div
          v-if="expenseMatchedItems.length"
          class="finance-asset-page__detail-block finance-asset-page__detail-block--dialog"
        >
          <div class="finance-asset-page__detail-block-title">命中明细</div>
          <div class="finance-asset-page__trace-list">
            <div
              v-for="(item, index) in expenseMatchedItems"
              :key="item.id || `expense-matched-${index}`"
              class="finance-asset-page__trace-item finance-asset-page__trace-item--matched"
            >
              <div class="finance-asset-page__trace-item-head">
                <strong>{{ item.itemName || '未命名明细' }}</strong>
                <el-tag size="small" effect="light" round type="success">精确命中</el-tag>
              </div>
              <div class="finance-asset-page__trace-item-grid">
                <div><span>金额</span><strong>{{ formatAmount(item.amount) }}</strong></div>
                <div><span>固定资产候选</span><strong>{{ item.assetCandidateFlag ? '是' : '否' }}</strong></div>
                <div class="finance-asset-page__detail-grid-span">
                  <span>备注</span>
                  <strong>{{ item.remark || '无' }}</strong>
                </div>
        ...
        <div class="finance-asset-page__detail-block finance-asset-page__detail-block--dialog">
          <div class="finance-asset-page__detail-block-title">费用明细</div>
          <div v-if="assetTrace.expense.items?.length" class="finance-asset-page__trace-list">
            <div
              v-for="(item, index) in assetTrace.expense.items"
              :key="item.id || `expense-item-${index}`"
              :class="['finance-asset-page__trace-item', { 'finance-asset-page__trace-item--matched': item.matched }]"
            >
              <div class="finance-asset-page__trace-item-head">
                <strong>{{ item.itemName || '未命名明细' }}</strong>
      ...
      <template #footer>
        <el-button
          v-if="canOpenOriginalSourceFromDialog"
          type="primary"
          plain
          :loading="navigatingSource"
          @click="jumpToSource(assetDetail?.sourceType, assetDetail?.sourceBizId, assetDetail?.sourceBizNo)"
        >
          打开原单据
        </el-button>
        <el-button @click="sourceDetailDialogOpen = false">关闭</el-button>
      </template>
    </Dialog>
  </div>
</template>
...
const sourceDetailDialogOpen = ref(false)
const sourceDetailType = ref<'purchase' | 'expense' | ''>('')
...
const canViewPurchaseInPage = checkPermi(['erp:purchase-in:query'])
const canViewExpensePage = checkPermi(['erp:finance-expense:query'])

const canJumpToSourceWithPermission = (sourceType?: number, sourceBizId?: number) => {
  if (!canJumpToSource(sourceType, sourceBizId)) {
    return false
  }
  if (sourceType === 10) {
    return canViewPurchaseInPage
  }
  if (sourceType === 20) {
    return canViewExpensePage
  }
  return false
}

const hasPurchaseSourceDetail = computed(() => Boolean(assetTrace.value?.purchaseIn))
const hasExpenseSourceDetail = computed(() => Boolean(assetTrace.value?.expense))
const canOpenSourceDetail = computed(() => hasPurchaseSourceDetail.value || hasExpenseSourceDetail.value)
const canOpenOriginalSourceFromDialog = computed(() =>
  canJumpToSourceWithPermission(assetDetail.value?.sourceType, assetDetail.value?.sourceBizId)
)
const canOpenSourceTraceEntry = computed(() => canOpenSourceDetail.value || canOpenOriginalSourceFromDialog.value)
const resolveSourceDetailType = (sourceType?: number): 'purchase' | 'expense' | '' => {
  if (sourceType === 10) return 'purchase'
  if (sourceType === 20) return 'expense'
  return ''
}
const sourceDetailDialogTitle = computed(() => {
  if (sourceDetailType.value === 'purchase') return '采购入库来源详情'
  if (sourceDetailType.value === 'expense') return '费用报销来源详情'
  return '来源详情'
})
const purchaseMatchedItems = computed(
  () => assetTrace.value?.purchaseIn?.items?.filter((item) => item.matched) || []
)
const expenseMatchedItems = computed(
  () => assetTrace.value?.expense?.items?.filter((item) => item.matched) || []
)
...
const openSourceDetailDialog = () => {
  if (hasPurchaseSourceDetail.value) {
    sourceDetailType.value = 'purchase'
    sourceDetailDialogOpen.value = true
    return
  }
  if (hasExpenseSourceDetail.value) {
    sourceDetailType.value = 'expense'
    sourceDetailDialogOpen.value = true
    return
  }
  if (canOpenOriginalSourceFromDialog.value) {
    sourceDetailType.value = resolveSourceDetailType(assetDetail.value?.sourceType)
    sourceDetailDialogOpen.value = true
    return
  }
  message.warning('当前资产暂无可查看的来源详情')
}

const resetSourceDetailDialog = () => {
  sourceDetailType.value = ''
}
...
const clearDetailDrawer = () => {
  sourceDetailDialogOpen.value = false
  sourceDetailType.value = ''
...
.finance-asset-page__detail-actions {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
  gap: 12px;
...
.finance-asset-page__context-card--dialog {
  margin-bottom: 20px;
}
...
.finance-asset-page__source-summary--dialog {
  margin-bottom: 20px;
}
...
.finance-asset-page__detail-block--dialog {
  margin-bottom: 16px;
}
...
.finance-asset-page__trace-list {
  display: flex;
  flex-direction: column;
  gap: 12px;
}

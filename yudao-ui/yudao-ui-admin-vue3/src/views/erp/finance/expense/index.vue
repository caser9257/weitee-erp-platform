      ...
      <div class="finance-shell__page-header">
        <div class="finance-shell__page-header-main">
          ...
          <div class="finance-shell__page-metrics">
            <span class="finance-shell__metric-chip">结果 {{ displayTotal }}</span>
    ...
    <ContentWrap class="finance-shell__filter-card">
      ...
      <el-form ref="queryFormRef" :model="queryParams" label-width="88px" class="finance-shell__query-form" @submit.prevent>
        ...
        <div v-if="advancedExpanded" class="finance-shell__query-grid finance-shell__query-grid--wide">
          ...
          <el-form-item label="研发口径" prop="rdAccountingType">
            <el-select v-model="queryParams.rdAccountingType" placeholder="请选择研发口径" clearable class="!w-full">
              <el-option
                v-for="item in ERP_FINANCE_EXPENSE_RD_ACCOUNTING_TYPE_OPTIONS"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
            ...
            <el-table-column min-width="220">
              ...
              <template #default="{ row }">
                <div class="finance-shell__primary-cell">
                  ...
                  <div v-if="row.rdAccountingType" class="finance-shell__row-tags">
                    <span class="finance-shell__metric-pill finance-shell__metric-pill--warning">
                      {{ row.rdAccountingTypeName || (row.rdAccountingType === 10 ? '费用化' : '资本化') }}
                    </span>
                  </div>
                </div>
              </template>
            ...
            <el-table-column label="操作" fixed="right" align="center" width="280">
              <template #default="{ row }">
                <div class="finance-shell__row-actions">
                  ...
                  <el-button v-if="canUpdateStatus(row)" link type="success" v-hasPermi="['erp:finance-expense:update-status']" :disabled="actionBusy" @click.stop="handleUpdateStatus(row, 20)">审核</el-button>
                  <el-button v-if="canRollback(row)" link type="warning" v-hasPermi="['erp:finance-expense:update-status']" :disabled="actionBusy" @click.stop="handleUpdateStatus(row, 10)">反审核</el-button>
...
import {
  ERP_FINANCE_EXPENSE_RD_ACCOUNTING_TYPE_OPTIONS,
...
const filteredDemoExpenses = computed(() => {
  const keyword = String(queryParams.no || '')
    .trim()
    .toLowerCase()
  if (!keyword) {
    return demoExpenses
  }
  return demoExpenses.filter((item) => String(item.no || '').toLowerCase().includes(keyword))
})
const displayList = computed(() => (isDemoMode.value ? filteredDemoExpenses.value : list.value))
const displayTotal = computed(() => (isDemoMode.value ? filteredDemoExpenses.value.length : total.value))

const statusCount = computed(() => {
  const count = { process: 0, approve: 0, reject: 0 }
  const baseList = isDemoMode.value ? filteredDemoExpenses.value : list.value
...
const normalizeDateTimeValue = (value: unknown) => {
  ...
  if (typeof value === 'string') {
    ...
    return formatDate(/^\d+$/.test(trimmed) ? new Date(Number(trimmed)) : trimmed, 'YYYY-MM-DD HH:mm:ss')
  }
  return formatDate(new Date(value as any), 'YYYY-MM-DD HH:mm:ss')
}
const isValidBusinessDateTime = (value: unknown) => {
  const formatted = normalizeDateTimeValue(value)
  return Boolean(formatted) && !formatted.startsWith('1970-01-01')
}
...
const findDemoTrace = (id?: number | null) => (id ? demoTraceMap[id] : undefined)
const normalizeRouteNo = (value: unknown) => {
  if (Array.isArray(value)) {
    return value[0] ? String(value[0]).trim() : ''
  }
  return typeof value === 'string' ? value.trim() : ''
}
const syncRouteNoToQueryParams = () => {
  const routeNo = normalizeRouteNo(route.query.no)
  const changed = queryParams.no !== routeNo
  queryParams.no = routeNo
  if (changed) {
    queryParams.pageNo = 1
  }
  return changed
}
const clearRouteNoFilter = async () => {
  const hasRouteNo = route.query.no !== undefined
  if (queryParams.no) {
    queryParams.no = ''
    queryParams.pageNo = 1
  }
  ...
  try {
    ...
    if (demoExpense) {
      ...
      Object.assign(formModel, {
        id: demoExpense.id,
        no: demoExpense.no,
        status: demoExpense.status,
        expenseTime: demoExpense.expenseTime || '',
        expenseType: demoExpense.expenseType,
        rdAccountingType: demoExpense.rdAccountingType,
        deptId: demoExpense.deptId,
        projectId: demoExpense.projectId,
        supplierId: demoExpense.supplierId,
        financeUserId: demoExpense.financeUserId,
    ...
    Object.assign(formModel, {
      ...
      rdAccountingType: expense?.rdAccountingType,
...
const submitForm = async () => {
  ...
  const expenseTime = normalizeDateTimeValue(formModel.expenseTime)
  if (!isValidBusinessDateTime(expenseTime)) {
    message.warning('请选择报销时间')
    return
  }
...
const openDetailDialog = async (id?: number) => {
  ...
  try {
    const demoExpense = isLocalDemoHost() ? findDemoExpense(id) : undefined
    if (demoExpense) {
      Object.assign(detailData, demoExpense)
      Object.assign(traceData, findDemoTrace(id) || {})
      return
    }
    const [expenseResult, traceResult] = await Promise.allSettled([
      FinanceExpenseApi.getFinanceExpense(id),
      FinanceExpenseApi.getFinanceExpenseTrace(id)
    ])
    if (expenseResult.status === 'rejected') {
      throw expenseResult.reason
    }
    Object.assign(detailData, expenseResult.value || {})
    if (traceResult.status === 'fulfilled') {
      Object.assign(traceData, traceResult.value || {})
    }
  } catch (error: any) {
    const demoExpense = isLocalDemoHost() ? findDemoExpense(id) : undefined
    if (demoExpense) {
...
const handleUpdateStatus = async (row?: ErpFinanceExpenseVO, status?: number) => {
  const id = row?.id
  if (!id || !status || actionBusy.value) return
  if (status === 20 && !isValidBusinessDateTime(row?.expenseTime)) {
    message.warning('请先编辑并填写有效报销时间')
    return
  }
  try {

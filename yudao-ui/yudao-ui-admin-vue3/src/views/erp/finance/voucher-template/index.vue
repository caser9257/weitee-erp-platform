    ...
    <div class="voucher-template-page__shell">
      <div class="voucher-template-page__header">
        <div>
          ...
          <p class="voucher-template-page__subtitle">针对不同的业务类型，统一配置其对应的财务记账凭证规则与分录模板。</p>
        </div>
      ...
      <div class="voucher-template-page__filter-card">
        ...
        <div class="voucher-template-page__filter-grid">
          <div class="voucher-template-page__field">
            ...
            <el-input v-model="queryParams.name" clearable placeholder="输入名称关键词..." class="voucher-template-page__field-control" @keyup.enter="handleQuery">
              <template #prefix>
          ...
          <div class="voucher-template-page__field">
            <label class="voucher-template-page__field-label">账簿</label>
            <el-select v-model="queryParams.ledgerId" clearable filterable placeholder="全部账簿" class="voucher-template-page__field-control" :loading="ledgerLoading">
          ...
          <div class="voucher-template-page__field">
            ...
            <el-select v-model="queryParams.bizType" clearable placeholder="全部业务" class="voucher-template-page__field-control">
          ...
          <div class="voucher-template-page__field">
            ...
            <el-select v-model="queryParams.status" clearable placeholder="全部状态" class="voucher-template-page__field-control">
          ...
          <div class="voucher-template-page__field">
            ...
            <el-select v-model="queryParams.autoGenerate" clearable placeholder="全部方式" class="voucher-template-page__field-control">
      ...
      <div class="voucher-template-page__table-card">
        <div class="voucher-template-page__table-head">
          <div>
            <div class="voucher-template-page__table-title-row">
              ...
              <span class="voucher-template-page__table-filter-chip">已筛选 {{ list.length }} / {{ total }}</span>
            </div>
            <div class="voucher-template-page__section-subtitle">共 {{ total }} 条，当前数据直接来自后端分页接口。</div>
          </div>
          <div class="voucher-template-page__table-head-right">
            <label class="voucher-template-page__remark-switch">
              <el-checkbox v-model="showRemark">显示备注说明</el-checkbox>
            </label>
        ...
        <template v-else>
                ...
                <template #default="{ row }">
                  <div class="voucher-template-page__primary-cell">
                    ...
                    <div v-if="showRemark" class="voucher-template-page__primary-desc">
                      {{ row.defaultSummary || row.remark || '未配置说明' }}
                    </div>
                  </div>
                </template>
              </el-table-column>
          ...
          <div v-else class="voucher-template-page__empty">
            ...
            <div class="voucher-template-page__empty-text">当前条件下没有匹配结果，可以尝试重置筛选条件后重新查询。</div>
          </div>
        </template>
      ...
      <template v-else>
        ...
        <div v-if="detailData" class="voucher-template-page__detail-grid">
          <div class="voucher-template-page__detail-card">
            ...
            <div class="voucher-template-page__detail-value">{{ detailData.defaultSummary || '未设置' }}</div>
          </div>
          <div class="voucher-template-page__detail-card">
            <div class="voucher-template-page__detail-label">研发分类</div>
            <div class="voucher-template-page__detail-value">{{ detailData.researchCategoryName || getResearchCategoryLabel(detailData.researchCategory) || '未配置' }}</div>
          </div>
          <div class="voucher-template-page__detail-card">
      ...
      <el-form ref="dialogFormRef" :model="formData" :rules="formRules" label-width="110px" class="voucher-template-page__dialog-form">
        <div class="voucher-template-page__dialog-grid">
          <el-form-item label="账套" prop="ledgerId">
            <el-select v-model="formData.ledgerId" placeholder="请选择账套" filterable class="!w-full">
              <el-option v-for="item in ledgerOptions" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="默认摘要" prop="defaultSummary" class="voucher-template-page__dialog-span">
            <el-input v-model="formData.defaultSummary" placeholder="请输入默认摘要" />
          </el-form-item>
          <el-form-item label="研发分类" prop="researchCategory">
            <el-select v-model="formData.researchCategory" placeholder="请选择研发分类" clearable class="!w-full">

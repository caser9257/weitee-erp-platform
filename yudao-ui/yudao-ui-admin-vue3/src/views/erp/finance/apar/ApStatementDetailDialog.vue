    ...
    <template v-if="detail">
      <div class="finance-shell__section">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item :label="sourceOrderLabel">{{ detail.sourceOrderNo || '-' }}</el-descriptions-item>
      ...
      <div v-if="isOutsourceFeeStatement" class="finance-shell__section">
        <div class="finance-shell__section-head">
          <div class="finance-shell__section-title">委外加工费关联</div>
        </div>
        <div v-if="outsourceFeeErrorMessage" class="finance-shell__empty">
          <div class="finance-shell__empty-title">委外关联信息加载失败</div>
          <div class="finance-shell__empty-desc">请重试加载后查看关联信息</div>
          <el-button type="primary" plain :disabled="outsourceFeeLoading" @click="retryLoadOutsourceFeeDetail">
            重试
          </el-button>
        </div>
        <div v-else-if="outsourceFeeLoading" v-loading="true" class="ap-statement-detail__relation-card"></div>
        <el-descriptions v-else-if="outsourceFeeDetail" :column="2" border>
          <el-descriptions-item label="加工费单号">{{ outsourceFeeDetail.feeNo || '-' }}</el-descriptions-item>
          <el-descriptions-item label="来源委外单">{{ outsourceFeeDetail.orderNo || '-' }}</el-descriptions-item>
          <el-descriptions-item label="核销状态">
            {{ outsourceFeeDetail.reconciliationStatusName || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="登记时间">
            {{ formatDateValue(outsourceFeeDetail.feeTime as string) }}
          </el-descriptions-item>
          <el-descriptions-item label="加工费金额">
            {{ erpPriceInputFormatter(outsourceFeeDetail.feeAmount) }}
          </el-descriptions-item>
          <el-descriptions-item label="已核销金额">
            {{ erpPriceInputFormatter(outsourceFeeDetail.paidAmount) }}
          </el-descriptions-item>
          <el-descriptions-item label="剩余金额">
            {{ erpPriceInputFormatter(outsourceFeeDetail.remainAmount) }}
          </el-descriptions-item>
          <el-descriptions-item label="登记人">{{ outsourceFeeDetail.creatorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="备注" :span="2">{{ outsourceFeeDetail.remark || '-' }}</el-descriptions-item>
        </el-descriptions>
        <el-empty v-else description="暂无委外关联信息" />
      </div>
...
import { OutsourceFeeApi, type OutsourceFeeVO } from '@/api/erp/mrp/outsource-fee'
...
const outsourceFeeLoading = ref(false)
const outsourceFeeErrorMessage = ref('')
const outsourceFeeDetail = ref<OutsourceFeeVO>()
...
const isOutsourceFeeStatement = computed(() => detail.value?.bizType === 30)
const sourceOrderLabel = computed(() => (isOutsourceFeeStatement.value ? '来源委外单' : '来源采购单'))
...
const resetOutsourceFeeDetail = () => {
  outsourceFeeLoading.value = false
  outsourceFeeErrorMessage.value = ''
  outsourceFeeDetail.value = undefined
}
...
const loadOutsourceFeeDetail = async (statement: ApStatementVO, requestId?: number) => {
  resetOutsourceFeeDetail()
  if (statement.bizType !== 30 || !statement.bizId) {
    return
  }
  outsourceFeeLoading.value = true
  try {
    const data = await OutsourceFeeApi.getOutsourceFee(statement.bizId)
    if (requestId && (currentId.value !== requestId || !dialogVisible.value)) {
      return
    }
    outsourceFeeDetail.value = data
  } catch {
    outsourceFeeErrorMessage.value = '委外关联信息加载失败'
  } finally {
    outsourceFeeLoading.value = false
  }
}
...
const loadDetail = async (id: number) => {
  ...
  try {
    ...
    void loadOutsourceFeeDetail(data, id)
  } catch {
    detail.value = undefined
...
const retryLoadOutsourceFeeDetail = async () => {
  if (!detail.value) {
    return
  }
  await loadOutsourceFeeDetail(detail.value)
}
...
.ap-statement-detail__relation-card {
  min-height: 120px;
  border-radius: 12px;
}

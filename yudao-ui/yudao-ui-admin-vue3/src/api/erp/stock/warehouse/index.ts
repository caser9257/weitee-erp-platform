...
export interface WarehouseVO {
  id: number // 仓库编号
  name: string // 仓库名称
  categoryId?: number // 仓库分类编号
  categoryName?: string // 仓库分类名称
  address: string // 仓库地址
  sort: number // 排序
  remark: string // 备注
...
export interface WarehouseBatchUpdateReqVO {
  ids: number[]
...
export interface WarehouseBatchUpdateResultVO {
  successCount: number
  failureCount: number
  updatedIds: number[]
  failedItems: Array<{
...
export type WarehouseBatchFieldKey = 'address' | 'principal' | 'remark' | 'status' | 'sort'
...
export const WarehouseApi = {
  ...
  // 批量修改仓库
  batchUpdateWarehouse: async (data: WarehouseBatchUpdateReqVO) => {
    return await request.put({ url: `/erp/warehouse/batch-update`, data })
  },

...
export const requiresDualLedgerSourceValue = (sourceType?: number) => Number(sourceType) === 10
...
export const FinanceDualLedgerDiffConfigApi = {
  ...
  getDualLedgerDiffConfig: async (id: number) => {
    return await request.get<ErpFinanceDualLedgerDiffConfigVO>({
      url: '/erp/finance-dual-ledger-diff-config/get',
      params: { id }
    })
  },
  ...
  deleteDualLedgerDiffConfig: async (id: number) => {
    return await request.delete({ url: '/erp/finance-dual-ledger-diff-config/delete', params: { id } })
  },

        ...
        >
          {dictOptions.map((dict: DictDataType) => {
            if (valueArr.value.includes(dict.value)) {
              ...
              // [Global UI Optimization] Hardcode common variants to prevent monotonous color scheme
              const lbl = dict?.label || ''
              if (lbl === '开启' || lbl === '成功' || lbl === '正常') dict.colorType = 'success'
              if (lbl === '关闭' || lbl === '失败' || lbl === '停用') dict.colorType = 'danger'
              if (lbl === '系统' || lbl === '内置') dict.colorType = 'warning'
              if (lbl === '自定义') dict.colorType = '' 
              return (

...
import cn.weitee.erp.module.crm.controller.admin.business.vo.status.CrmBusinessStatusSaveReqVO;
import cn.weitee.erp.module.crm.dal.dataobject.business.CrmBusinessStatusDO;
import cn.weitee.erp.module.crm.dal.dataobject.business.CrmBusinessStatusTypeDO;
import cn.weitee.erp.module.crm.dal.mysql.business.CrmBusinessStatusMapper;
import cn.weitee.erp.module.crm.dal.mysql.business.CrmBusinessStatusTypeMapper;
...
public class CrmBusinessStatusServiceImpl implements CrmBusinessStatusService {
    ...
    @Resource
    private CrmBusinessStatusTypeMapper crmBusinessStatusTypeMapper;
    @Resource
    private CrmBusinessStatusMapper crmBusinessStatusMapper;
    ...
    public Long createBusinessStatus(CrmBusinessStatusSaveReqVO createReqVO) {
        ...
        crmBusinessStatusTypeMapper.insert(statusType);
        // 2.2 插入状态
        ...
        crmBusinessStatusMapper.insertBatch(statuses);
    ...
    public void updateBusinessStatus(CrmBusinessStatusSaveReqVO updateReqVO) {
        ...
        crmBusinessStatusTypeMapper.updateById(updateObj);
        // 2.2 更新状态
    ...
    private void updateBusinessStatus(Long id, List<CrmBusinessStatusDO> newList) {
        List<CrmBusinessStatusDO> oldList = crmBusinessStatusMapper.selectListByTypeId(id);
        ...
        if (CollUtil.isNotEmpty(diffList.get(0))) {
            ...
            crmBusinessStatusMapper.insertBatch(diffList.get(0));
        }
        if (CollUtil.isNotEmpty(diffList.get(1))) {
            crmBusinessStatusMapper.updateBatch(diffList.get(1));
        }
        if (CollUtil.isNotEmpty(diffList.get(2))) {
            crmBusinessStatusMapper.deleteByIds(convertSet(diffList.get(2), CrmBusinessStatusDO::getId));
        }
    }

    private void validateBusinessStatusTypeExists(Long id) {
        if (crmBusinessStatusTypeMapper.selectById(id) == null) {
            throw exception(BUSINESS_STATUS_TYPE_NOT_EXISTS);
        }
    }

    private void validateBusinessStatusTypeNameUnique(String name, Long id) {
        CrmBusinessStatusTypeDO statusType = crmBusinessStatusTypeMapper.selectByName(name);
        if (statusType == null
    ...
    public void deleteBusinessStatusType(Long id) {
        ...
        // 2.1 删除类型
        crmBusinessStatusTypeMapper.deleteById(id);
        // 2.2 删除状态
        crmBusinessStatusMapper.deleteByTypeId(id);
    }

    @Override
    public CrmBusinessStatusTypeDO getBusinessStatusType(Long id) {
        return crmBusinessStatusTypeMapper.selectById(id);
    }
    ...
    public List<CrmBusinessStatusTypeDO> getBusinessStatusTypeList() {
        return crmBusinessStatusTypeMapper.selectList();
    }

    @Override
    public PageResult<CrmBusinessStatusTypeDO> getBusinessStatusTypePage(PageParam pageReqVO) {
        return crmBusinessStatusTypeMapper.selectPage(pageReqVO);
    }
    ...
    public List<CrmBusinessStatusTypeDO> getBusinessStatusTypeList(Collection<Long> ids) {
        ...
        return crmBusinessStatusTypeMapper.selectByIds(ids);
    }

    @Override
    public List<CrmBusinessStatusDO> getBusinessStatusListByTypeId(Long typeId) {
        List<CrmBusinessStatusDO> list = crmBusinessStatusMapper.selectListByTypeId(typeId);
    ...
    public List<CrmBusinessStatusDO> getBusinessStatusList(Collection<Long> ids) {
        ...
        return crmBusinessStatusMapper.selectByIds(ids);
    }

    @Override
    public CrmBusinessStatusDO getBusinessStatus(Long id) {
        return crmBusinessStatusMapper.selectById(id);
    }

    @Override
    public CrmBusinessStatusDO validateBusinessStatus(Long statusTypeId, Long statusId) {
        CrmBusinessStatusDO status = crmBusinessStatusMapper.selectByTypeIdAndId(statusTypeId, statusId);
        if (status == null) {

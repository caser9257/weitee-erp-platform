...
import cn.weitee.erp.module.crm.controller.admin.statistics.vo.funnel.*;
import cn.weitee.erp.module.crm.dal.dataobject.business.CrmBusinessDO;
import cn.weitee.erp.module.crm.dal.mysql.statistics.CrmStatisticsFunnelMapper;
import cn.weitee.erp.module.crm.enums.business.CrmBusinessEndStatusEnum;
...
public class CrmStatisticsFunnelServiceImpl implements CrmStatisticsFunnelService {
    ...
    @Resource
    private CrmStatisticsFunnelMapper crmStatisticsFunnelMapper;
    ...
    public CrmStatisticFunnelSummaryRespVO getFunnelSummary(CrmStatisticsFunnelReqVO reqVO) {
        ...
        // 2. 获得漏斗数据
        Long customerCount = crmStatisticsFunnelMapper.selectCustomerCountByDate(reqVO);
        Long businessCount = crmStatisticsFunnelMapper.selectBusinessCountByDateAndEndStatus(reqVO, null);
        Long businessWinCount = crmStatisticsFunnelMapper.selectBusinessCountByDateAndEndStatus(reqVO, CrmBusinessEndStatusEnum.WIN.getStatus());
    ...
    public List<CrmStatisticsBusinessSummaryByEndStatusRespVO> getBusinessSummaryByEndStatus(CrmStatisticsFunnelReqVO reqVO) {
        ...
        // 2. 获得统计数据
        return crmStatisticsFunnelMapper.selectBusinessSummaryListGroupByEndStatus(reqVO);
    }
    ...
    public List<CrmStatisticsBusinessSummaryByDateRespVO> getBusinessSummaryByDate(CrmStatisticsFunnelReqVO reqVO) {
        ...
        // 2. 按天统计，获取分项统计数据
        List<CrmStatisticsBusinessSummaryByDateRespVO> businessSummaryList = crmStatisticsFunnelMapper.selectBusinessSummaryGroupByDate(reqVO);
        // 3. 按照日期间隔，合并数据
    ...
    public List<CrmStatisticsBusinessInversionRateSummaryByDateRespVO> getBusinessInversionRateSummaryByDate(CrmStatisticsFunnelReqVO reqVO) {
        ...
        // 2. 按天统计，获取分项统计数据
        List<CrmStatisticsBusinessInversionRateSummaryByDateRespVO> businessSummaryList = crmStatisticsFunnelMapper.selectBusinessInversionRateSummaryByDate(reqVO);
        // 3. 按照日期间隔，合并数据
        List<LocalDateTime[]> timeRanges = LocalDateTimeUtils.getDateRangeList(reqVO.getTimes()[0], reqVO.getTimes()[1], reqVO.getInterval());
        return convertList(timeRanges, times -> {

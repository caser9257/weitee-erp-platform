package cn.weitee.erp.module.crm.service.permission;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.core.util.ObjUtil;
import cn.weitee.erp.framework.common.util.collection.CollectionUtils;
import cn.weitee.erp.framework.common.util.object.BeanUtils;
import cn.weitee.erp.module.crm.controller.admin.permission.vo.CrmPermissionSaveReqVO;
import cn.weitee.erp.module.crm.controller.admin.permission.vo.CrmPermissionUpdateReqVO;
...
import cn.weitee.erp.module.crm.dal.dataobject.permission.CrmPermissionDO;
import cn.weitee.erp.module.crm.dal.mysql.permission.CrmPermissionMapper;
...
import cn.weitee.erp.module.crm.util.CrmPermissionUtils;
...
import static cn.weitee.erp.module.crm.enums.permission.CrmPermissionLevelEnum.isOwner;
...
public class CrmPermissionServiceImpl implements CrmPermissionService {
    ...
    public void updatePermission(CrmPermissionUpdateReqVO updateReqVO) {
        ...
        crmPermissionMapper.updateBatch(updateList);
    }

    private void validatePermissionExists(Collection<Long> ids) {
        List<CrmPermissionDO> permissionList = crmPermissionMapper.selectByIds(ids);
    ...
    private void validatePermissionNotExists(Collection<CrmPermissionCreateReqBO> createReqBOs) {
        ...
        Long count = crmPermissionMapper.selectListByBiz(bizTypes, bizIds, userIds);
        if (count > 0) {
    ...
    public void deletePermission(Integer bizType, Long bizId, Integer level) {
        ...
        // 删除数据权限
        crmPermissionMapper.deleteByIds(convertSet(permissions, CrmPermissionDO::getId));
    }
    ...
    public void deletePermissionBatch(Collection<Long> ids, Long userId) {
        List<CrmPermissionDO> permissions = crmPermissionMapper.selectByIds(ids);
        if (CollUtil.isEmpty(permissions)) {
            throw exception(CRM_PERMISSION_NOT_EXISTS);
        }
        ...
        // 校验操作人是否为负责人
        CrmPermissionDO permission = crmPermissionMapper.selectByBizAndUserId(permissions.get(0).getBizType(), permissions.get(0).getBizId(), userId);
        if (permission == null) {
        ...
        // 删除数据权限
        crmPermissionMapper.deleteByIds(ids);
    }

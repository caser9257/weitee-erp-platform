...
import cn.weitee.erp.module.crm.controller.admin.business.vo.business.CrmBusinessTransferReqVO;
import cn.weitee.erp.module.crm.controller.admin.contact.vo.CrmContactTransferReqVO;
import cn.weitee.erp.module.crm.controller.admin.contract.vo.contract.CrmContractTransferReqVO;
import cn.weitee.erp.module.crm.controller.admin.customer.vo.customer.*;
import cn.weitee.erp.module.crm.dal.dataobject.business.CrmBusinessDO;
import cn.weitee.erp.module.crm.dal.dataobject.contact.CrmContactDO;
import cn.weitee.erp.module.crm.dal.dataobject.contract.CrmContractDO;
import cn.weitee.erp.module.crm.dal.dataobject.customer.CrmCustomerDO;
import cn.weitee.erp.module.crm.dal.dataobject.customer.CrmCustomerLimitConfigDO;
import cn.weitee.erp.module.crm.dal.dataobject.customer.CrmCustomerPoolConfigDO;
import cn.weitee.erp.module.crm.dal.mysql.customer.CrmCustomerMapper;
import cn.weitee.erp.module.crm.enums.common.CrmBizTypeEnum;
import cn.weitee.erp.module.crm.enums.common.CrmSceneTypeEnum;
import cn.weitee.erp.module.crm.enums.permission.CrmPermissionLevelEnum;
import cn.weitee.erp.module.crm.framework.permission.core.annotations.CrmPermission;
import cn.weitee.erp.module.crm.service.business.CrmBusinessService;
import cn.weitee.erp.module.crm.service.contact.CrmContactService;
import cn.weitee.erp.module.crm.service.contract.CrmContractService;
import cn.weitee.erp.module.crm.service.customer.bo.CrmCustomerCreateReqBO;
import cn.weitee.erp.module.crm.service.permission.CrmPermissionService;
import cn.weitee.erp.module.crm.service.permission.bo.CrmPermissionCreateReqBO;
import cn.weitee.erp.module.crm.service.permission.bo.CrmPermissionTransferReqBO;
...
import static cn.weitee.erp.framework.common.util.collection.CollectionUtils.filterList;
import static cn.weitee.erp.module.crm.enums.ErrorCodeConstants.*;
import static cn.weitee.erp.module.crm.enums.LogRecordConstants.*;
import static cn.weitee.erp.module.crm.enums.customer.CrmCustomerLimitConfigTypeEnum.CUSTOMER_LOCK_LIMIT;
import static cn.weitee.erp.module.crm.enums.customer.CrmCustomerLimitConfigTypeEnum.CUSTOMER_OWNER_LIMIT;
...
public class CrmCustomerServiceImpl implements CrmCustomerService {
    ...
    public void updateCustomer(CrmCustomerSaveReqVO updateReqVO) {
        ...
        crmCustomerMapper.updateById(updateObj);
    ...
    public void updateCustomerDealStatus(Long id, Boolean dealStatus) {
        ...
        // 2. 更新客户的成交状态
        crmCustomerMapper.updateById(new CrmCustomerDO().setId(id).setDealStatus(dealStatus));
    ...
    public void updateCustomerFollowUp(Long id, LocalDateTime contactNextTime, String contactLastContent) {
        ...
        // 2. 更新客户的跟进信息
        crmCustomerMapper.updateById(new CrmCustomerDO().setId(id).setFollowUpStatus(true).setContactNextTime(contactNextTime)
    ...
    public void transferCustomer(CrmCustomerTransferReqVO reqVO, Long userId) {
        ...
        // 2.2 转移后重新设置负责人
        crmCustomerMapper.updateById(new CrmCustomerDO().setId(reqVO.getId())
    ...
    public void lockCustomer(CrmCustomerLockReqVO lockReqVO, Long userId) {
        ...
        // 2. 更新锁定状态
        crmCustomerMapper.updateById(BeanUtils.toBean(lockReqVO, CrmCustomerDO.class));
    ...
    public CrmCustomerImportRespVO importCustomerList(List<CrmCustomerImportExcelVO> importCustomers,
        ...
        importCustomers.forEach(importCustomer -> {
            ...
            // 情况一：判断如果不存在，在进行插入
            CrmCustomerDO existCustomer = crmCustomerMapper.selectByCustomerName(importCustomer.getName());
            ...
            crmCustomerMapper.updateById(updateCustomer);
    ...
    public void putCustomerPool(Long id) {
        // 1. 校验存在
        CrmCustomerDO customer = crmCustomerMapper.selectById(id);
        if (customer == null) {
    ...
    public void receiveCustomer(List<Long> ids, Long ownerUserId, Boolean isReceive) {
        // 1.1 校验存在
        List<CrmCustomerDO> customers = crmCustomerMapper.selectByIds(ids);
        ...
        // 2.2 更新客户负责人
        crmCustomerMapper.updateBatch(updateCustomers);
        // 2.3 创建负责人数据权限
    ...
    public int autoPutCustomerPool() {
        ...
        // 1. 获得需要放到的客户列表
        List<CrmCustomerDO> customerList = crmCustomerMapper.selectListByAutoPool(poolConfig);
        // 2. 逐个放入公海
        int count = 0;
    ...
    protected void putCustomerPool(CrmCustomerDO customer) {
        // 1. 设置负责人为 NULL
        int updateOwnerUserIncr = crmCustomerMapper.updateOwnerUserIdById(customer.getId(), null);
    ...
    public List<CrmCustomerDO> getCustomerList(Collection<Long> ids) {
        ...
        return crmCustomerMapper.selectByIds(ids);
    }

    @Override
    public PageResult<CrmCustomerDO> getCustomerPage(CrmCustomerPageReqVO pageReqVO, Long userId) {
        return crmCustomerMapper.selectPage(pageReqVO, userId);
    }
    ...
    public PageResult<CrmCustomerDO> getPutPoolRemindCustomerPage(CrmCustomerPageReqVO pageVO, Long userId) {
        ...
        return crmCustomerMapper.selectPutPoolRemindCustomerPage(pageVO, poolConfig, userId);
    }
    ...
    public Long getPutPoolRemindCustomerCount(Long userId) {
        ...
        return crmCustomerMapper.selectPutPoolRemindCustomerCount(pageVO, poolConfig, userId);
    }

    @Override
    public Long getTodayContactCustomerCount(Long userId) {
        return crmCustomerMapper.selectCountByTodayContact(userId);
    }

    @Override
    public Long getFollowCustomerCount(Long userId) {
        return crmCustomerMapper.selectCountByFollow(userId);
    }
    ...
    private CrmCustomerDO validateCustomerExists(Long id) {
        CrmCustomerDO customerDO = crmCustomerMapper.selectById(id);
        if (customerDO == null) {
            throw exception(CUSTOMER_NOT_EXISTS);
        }
        return customerDO;
    ...
    private void validateCustomerExceedOwnerLimit(Long userId, int newCount) {
        ...
        Long ownerCount = crmCustomerMapper.selectCountByDealStatusAndOwnerUserId(null, userId);
        Long dealOwnerCount = crmCustomerMapper.selectCountByDealStatusAndOwnerUserId(true, userId);
    ...
    private void validateCustomerExceedLockLimit(Long userId) {
        ...
        Long lockCount = crmCustomerMapper.selectCountByLockStatusAndOwnerUserId(true, userId);

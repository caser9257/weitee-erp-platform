...
import cn.weitee.erp.module.erp.controller.admin.stock.vo.check.ErpStockCheckRespVO;
...
import cn.weitee.erp.module.erp.service.stock.ErpStockCheckService;
...
public class ErpStockCheckController {
    ...
    @Operation(summary = "更新库存盘点单的状态（已废弃，请使用专用端点）")
    ...
    private PageResult<ErpStockCheckRespVO> buildStockCheckVOPageResult(PageResult<ErpStockCheckDO> pageResult) {
        ...
        Map<Long, AdminUserRespDTO> userMap = adminUserApi.getUserMap(
                convertSet(pageResult.getList(), stockCheck -> parseUserId(stockCheck.getCreator())));
        // 2. 开始拼接
        return BeanUtils.toBean(pageResult, ErpStockCheckRespVO.class, stockCheck -> {
            ...
            MapUtils.findAndThen(userMap, parseUserId(stockCheck.getCreator()), user -> stockCheck.setCreatorName(user.getNickname()));
        });
    }

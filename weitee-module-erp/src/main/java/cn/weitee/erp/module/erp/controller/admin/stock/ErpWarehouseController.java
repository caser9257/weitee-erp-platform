...
import cn.weitee.erp.module.erp.controller.admin.stock.vo.warehouse.ErpWarehouseBatchUpdateReqVO;
import cn.weitee.erp.module.erp.controller.admin.stock.vo.warehouse.ErpWarehouseBatchUpdateResultVO;
import cn.weitee.erp.module.erp.controller.admin.stock.vo.warehouse.ErpWarehousePageReqVO;
import cn.weitee.erp.module.erp.controller.admin.stock.vo.warehouse.ErpWarehouseRespVO;
...
import cn.weitee.erp.module.erp.service.stock.ErpWarehouseCategoryService;
...
public class ErpWarehouseController {
    ...
    @Operation(summary = "批量更新仓库")
    @PreAuthorize("@ss.hasPermission('erp:warehouse:update')")
    public CommonResult<ErpWarehouseBatchUpdateResultVO> updateWarehouseBatch(
            @Valid @RequestBody ErpWarehouseBatchUpdateReqVO reqVO) {
        return success(warehouseService.updateWarehouseBatch(reqVO));
    }
    ...
    public CommonResult<PageResult<ErpWarehouseRespVO>> getWarehousePage(@Valid ErpWarehousePageReqVO pageReqVO) {
        ...
        return success(BeanUtils.toBean(pageResult, ErpWarehouseRespVO.class, this::fillCategoryName));
    }
    ...
    public void exportWarehouseExcel(@Valid ErpWarehousePageReqVO pageReqVO,
        ...
        ExcelUtils.write(response, "仓库.xls", "数据", ErpWarehouseRespVO.class,
                BeanUtils.toBean(list, ErpWarehouseRespVO.class, this::fillCategoryName));
    }

    private void fillCategoryName(ErpWarehouseRespVO respVO) {
        if (respVO.getCategoryId() == null) {
            return;
        }
        var category = warehouseCategoryService.getWarehouseCategory(respVO.getCategoryId());
        if (category != null) {
